      ...
      if (state.step === 4) {
        ...
        if (textInput) {
          ...
          textInput.oninput = (e) => {
            ...
            // ДЕБАУНС re-render. Без debounce каждое нажатие клавиши
            // обновляет flatPreview.src (новый Konva-PNG). На мобиле смена
            // <img>.src вызывает реdraw, который сбрасывает browser native
            // pinch-zoom юзера. Дебаунсим: пока юзер активно печатает —
            // только лёгкий sync (counter / hint), после паузы 250ms —
            // полный re-render Konva flatPreview.
            if (typeof window.__textInputRerenderTimer !== 'undefined') {
              clearTimeout(window.__textInputRerenderTimer);
            }
            // Лёгкий sync для UI-индикаторов (счётчик символов и т.д.)
            const lightSyncContainer = document.getElementById('stepContent');
            if (lightSyncContainer) {
              const textBreakdown = getPreviewTextBreakdown(state.textValue, template, printSize);
              const textRules = getPreviewTextRules(template, printSize);
              const linesEl = lightSyncContainer.querySelector('#textLinesCount');
              if (linesEl) linesEl.textContent = `${textBreakdown.lines.length}/${textRules.maxLines} строк`;
              const overflowHint = lightSyncContainer.querySelector('#textOverflowHint');
              if (overflowHint) overflowHint.hidden = !textBreakdown.truncated;
              const fontWarning = lightSyncContainer.querySelector('#fontWarning');
              if (fontWarning) {
                const shouldWarn = isFontLatinOnly(state.font, template) && textContainsCyrillic(state.textValue);
                fontWarning.hidden = !shouldWarn;
              }
              const embroSym = lightSyncContainer.querySelector('#embroiderySymbolsCount');
              if (embroSym) embroSym.textContent = String(getEmbroiderySymbolsCount());
              const embroPrice = lightSyncContainer.querySelector('#embroideryPriceValue');
              if (embroPrice) embroPrice.textContent = formatPrice(getEmbroideryTotal());
            ...
            schedulePersistenceSave();
            // Тяжёлый Konva re-render — debounced.
            const savedCropZoom = state.cropZoom;
            const savedCropPosition = { ...state.cropPosition };
            window.__textInputRerenderTimer = setTimeout(() => {
              window.__textInputRerenderTimer = undefined;
              updateStepFourControls();
              // Защита от случайного сброса cropZoom внутри updateStepFourControls.
