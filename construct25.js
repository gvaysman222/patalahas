...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    // Блокируем iOS-специфичные события native pinch (gesturestart/change/end).
    // touch-action: pan-y этого НЕ блокирует — это отдельный API Safari.
    // Если не блокировать, Safari всё равно делает свой visual viewport zoom,
    // а наш pointer-handler получает обрезанные события → state.cropZoom
    // не обновляется → после text reflow Safari сбрасывает visual zoom →
    // пользователь видит «zoom reset».
    if (typeof document !== 'undefined') {
      const blockGesture = (e) => { try { e.preventDefault(); } catch (_) {} };
