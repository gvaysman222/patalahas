...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    async function submitOrderForm(event) {
      ...
      try {
        ...
        console.log(`[CCK-UPLOAD] Оформление товара №${committedItemNumber} (orderId=${state.orderId}). Описание зафиксировано. Всего товаров в сводке: ${cartOrderSummariesByOrderId.size}.`);
...
// ===== constructor-checkout-mvp-tilda-js-1-preview.html =====
    ...
    async function syncTildaUploadFields(tildaForm) {
      ...
      const sourceOriginalFiles = (customMode ? state.uploads : state.uploads.slice(0, 1))
        ...
        .filter(Boolean);
      console.log(`[CCK-UPLOAD] === Прикрепление файлов товара (orderId=${state.orderId}, customMode=${customMode}) ===`);
      console.log(`[CCK-UPLOAD] Исходных фото: ${sourceOriginalFiles.length}`, sourceOriginalFiles.map((file) => file?.name || '(без имени)'));
      const preparedOriginalFiles = await normalizeFilesForTildaUpload(sourceOriginalFiles);
      console.log(`[CCK-UPLOAD] Готовлю превью: ${customMode ? 'нет (кастом)' : 'да'}`);
      ...
      console.log(`[constructor] Файлы текущего товара добавлены к набору корзины. Всего в наборе: ${filesForTilda.length} шт.`, filesForTilda.map((file) => file?.name || '(без имени)'));
      ...
      await withPreservedTildaCartFormValues(async () => {
        ...
        if (upwidgetContainer && filesForTilda.length) {
          ...
          if (rendered) {
            console.log(`[constructor] Файлы прикреплены к корзине Tilda (upwidget): ${attached}/${total}.`, fileNames);
            return;
          }
          ...
          console.log('[constructor] Нет файлов для прикрепления к корзине Tilda.');
        } else {
        ...
        if (synced) {
          console.log(`[constructor] Файлы успешно прикреплены к корзине Tilda (file input): ${filesForTilda.length} шт.`, fileNames);
        } else {
...
function getTildaCartForm() {
    ...
    async function uploadCandidateToCdnWithRetries(container, key, candidate, maxAttempts = 2) {
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        ...
        try {
          ...
          console.warn(`[CCK-UPLOAD] Ошибка заливки ${candidate.name} (попытка ${attempt}): ${msg}`);
    ...
    async function ensureCartFileUploadedToCdn(container, file) {
      ...
      if (uploadedCdnFilesByKey.has(key)) {
        console.log(`[CCK-UPLOAD] Файл уже в кэше CDN, повторная заливка не требуется: ${file.name}`);
        return uploadedCdnFilesByKey.get(key);
      }
      if ((cdnUploadAttemptsByKey.get(key) || 0) >= 9) {
        console.warn(`[CCK-UPLOAD] Файл превысил лимит попыток заливки, пропускаю: ${file.name}`);
        return null;
      }
      console.log(`[CCK-UPLOAD] Заливаю файл в Tilda CDN: ${file.name} (${Math.round((file.size || 0) / 1024)} КБ, тип ${file.type || '—'})`);
      ...
      if (!uploaded) {
        console.warn(`[CCK-UPLOAD] ${forbidden ? 'Тип отклонён Tilda' : 'Заливка не удалась'} — переконвертирую ${file.name} в чистый JPEG и повторяю.`);
        ...
        if (jpeg) {
          ...
          console.log(`[CCK-UPLOAD] Переконвертировано в JPEG: ${file.name} (${Math.round((normalized.size || 0) / 1024)} КБ). Повторная заливка.`);
          ({ uploaded } = await uploadCandidateToCdnWithRetries(container, key, normalized));
          if (uploaded) console.log(`[CCK-UPLOAD] ✓ После переконвертации файл принят Tilda: ${file.name}`);
        } else {
          console.error(`[CCK-UPLOAD] Не удалось переконвертировать ${file.name} в JPEG (браузер не смог декодировать).`);
        }
      }
      if (uploaded) {
        ...
        console.log(`[CCK-UPLOAD] ✓ Файл загружен в CDN: ${file.name} → ${uploaded.value}`);
        return entry;
      }
      console.error(`[CCK-UPLOAD] ✗ Файл НЕ загружен после попыток (в т.ч. переконвертации): ${file.name}`);
      return null;
    }
    ...
    async function attachTrackedCartFilesToUpwidget(container) {
      const trackedFiles = getCartUploadFilesSnapshot();
      console.log(`[CCK-UPLOAD] attach: в наборе ${trackedFiles.length} файл(ов):`, trackedFiles.map((file) => file?.name || '(без имени)'));
      if (!container) {
        console.warn('[CCK-UPLOAD] attach: контейнер upwidget не передан.');
      ...
      console.log(`[CCK-UPLOAD] attach завершён: отрисовано=${rendered}, прикреплено ${items.length}/${trackedFiles.length}` + (failed.length ? `, не удалось: ${failed.join(', ')}` : ''));
      if (failed.length) {
        try { window.__cckLog?.('cart_upload_api_failed', { files: failed }); } catch (_) {}
        console.error('[CCK-UPLOAD] Не удалось загрузить файлы в Tilda CDN после повторов:', failed);
      }
    ...
    async function syncCartUploadFilesWithActiveCart() {
      ...
      try {
        pruneCartUploadFilesByActiveCart();
        ...
        if (!trackedFiles.length) {
          ...
          console.log('[CCK-UPLOAD] Корзина пуста — кэш файлов, описания и нумерация сброшены.');
        ...
        // Поддерживаем актуальной сводку «Данные заказа» по всем товарам — на случай
        // перерисовки корзины Tilda между добавлениями.
        writeCombinedOrderSummaryToTildaForm(targetForm);
        // В форме уже есть весь набор файлов — больше ничего не делаем.
        if (!getMissingTrackedCartUploadFiles(targetForm).length) return false;
        return await withPreservedTildaCartFormValues(async () => {
          ...
          if (container) {
            ...
            try { window.__cckLog?.('cart_upload_reconciled', { attached, total, failed }); } catch (_) {}
            console.log(`[constructor] Корзина пересобрана из кэша: ${attached}/${total} файлов.`);
