    ...
    async function syncCartUploadFilesWithActiveCart() {
      ...
      try {
        ...
        return await withPreservedTildaCartFormValues(async () => {
          ...
          if (container) {
            // attach перерисовывает виджет ПОЛНЫМ актуальным набором из кэша —
            // тем самым удаляя файлы товара, которого больше нет в корзине.
            const { rendered, attached, total, failed } = await attachTrackedCartFilesToUpwidget(container);
            try { window.__cckLog?.('cart_upload_reconciled', { attached, total, failed, changed, hasStaleInWidget }); } catch (_) {}
