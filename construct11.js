...
function renderLeftSummary() {
    ...
    function renderVisualizer() {
      ...
      // Warm all variant images for the active product so subsequent color
      // switches don't trigger a network round-trip while the spinner is up.
      scheduleProductImagesPreload(selectedProduct?.id);
      ...
      if (isKonvaAvailable()) {
        ...
        if (KONVA_DEV_MODE) {
          ...
          if (sceneKey !== konvaPreviewLastKey) {
            ...
            // Show the spinner for the entire async pipeline (image fetches +
            // canvas export + decode of the new bitmap) so the UI doesn't
            // appear to freeze on product / template / color switches.
            // Uses a dedicated flag so resetCheckoutCompositePreview() (called
            // later in renderVisualizer) can't clear it.
