    ...
    function hydrateTemplatePreviewIcons(root = document.getElementById('stepContent')) {
      ...
      slots.forEach((slot) => {
        ...
        if (template && iconSrcRaw) {
          ...
          art.innerHTML = `<img src="${escapeAttr(displayUrl)}" alt="${escapeAttr(template.name)}" class="template-choice-icon-preview-img" loading="eager" decoding="async" />`;
      ...
      pendingSlots.forEach(({ slot, art, template, promise }) => {
        promise.then((dataUrl) => {
          ...
          art.innerHTML = `<img src="${escapeAttr(dataUrl || fallbackUrl)}" alt="${escapeAttr(template.name)}" class="template-choice-icon-preview-img" />`;
          slot.classList.remove('is-loading');
          slot.classList.add('is-ready');
        });
      });
      ...
      if (!hideDesign && state.selectedCustom) {
        ...
        if (customTpl && customTpl.previewImage) {
          ...
          html = `
            ...
            >
              <img src="${escapeAttr(normalizeProductImageUrl(customTpl.previewImage))}" alt="preview" style="object-fit:contain; object-position:center center; width:100%; height:100%; pointer-events:none; user-select:none;">
            </div>
          `;
        }
