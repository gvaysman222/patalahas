...
(function initConstructorCheckoutTemplateConfig(global) {
  ...
  global.CONSTRUCTOR_CHECKOUT_TEMPLATE_CONFIG = {
    ...
    customTemplates: [
      { id: 'custom-1', name: 'Парный полароид', note: 'Разработка дизайнером', price: 1300, previewImage: 'https://www.dropbox.com/scl/fo/32sdavmje7anx9fg8h13a/ADGeI8JB8vH2CU-zKFP2-9w/001.png?rlkey=hko8iad8atbm6bukwynhqlvps&st=fhiycxfb&raw=1', photoDefaults: { exportX: 407, exportY: 481.1, width: 192.1, height: 128.8, scale: 2.183 } },
      { id: 'custom-2', name: 'Силуэт', note: 'Разработка дизайнером', price: 1550, previewImage: 'https://www.dropbox.com/scl/fo/32sdavmje7anx9fg8h13a/APT4Bvd9uJzDm2uWBDiqHmk/002.png?rlkey=hko8iad8atbm6bukwynhqlvps&st=3sdpmecg&raw=1', photoDefaults: { exportX: 431, exportY: 479.7, width: 155.4, height: 171.2, scale: 1.766 } },
      { id: 'custom-3', name: 'Голова в колпаке + фраза', note: 'Разработка дизайнером', price: 1350, previewImage: 'https://www.dropbox.com/scl/fo/32sdavmje7anx9fg8h13a/AKzgf8MyTITN8tOBkPRvLEk/003.png?rlkey=hko8iad8atbm6bukwynhqlvps&st=1qcfqkq1&raw=1', photoDefaults: { exportX: 427.3, exportY: 456, width: 192.3, height: 206, scale: 2.185 } },
      { id: 'custom-4', name: 'Фраза из фото', note: 'Разработка дизайнером', price: 1350, previewImage: 'https://www.dropbox.com/scl/fo/32sdavmje7anx9fg8h13a/AH_cKJkhh_G52dcEwLxF2PA/004.png?rlkey=hko8iad8atbm6bukwynhqlvps&st=k7ryeajv&raw=1', photoDefaults: { exportX: 373.5, exportY: 481.7, width: 275.5, height: 85.6, scale: 3.131 } },
      { id: 'custom-5', name: 'Коллаж из 3-х фото', note: 'Разработка дизайнером', price: 1700, previewImage: 'https://www.dropbox.com/scl/fo/32sdavmje7anx9fg8h13a/ALmdTR-0Axc6r3oD1b9iMt8/5.%20%D0%9A%D0%BE%D0%BB%D0%BB%D0%B0%D0%B6%20%D0%B8%D0%B7%203-%D1%85%20%D1%84%D0%BE%D1%82%D0%BE.png?rlkey=hko8iad8atbm6bukwynhqlvps&st=eviyphsl&raw=1', photoDefaults: { exportX: 373.2, exportY: 480.2, width: 262.9, height: 102.2, scale: 2.987 } },
      { id: 'custom-6', name: 'Коллаж из 4-х фото', note: 'Разработка дизайнером', price: 2100, previewImage: 'https://www.dropbox.com/scl/fo/32sdavmje7anx9fg8h13a/ADU0wBCowM9BTqActIBrbzk/006.png?rlkey=hko8iad8atbm6bukwynhqlvps&st=fk9c1ez0&raw=1', photoDefaults: { exportX: 423.2, exportY: 480, width: 168, height: 227.2, scale: 2.058 } },
    ],
...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    async function preloadInitialRenderImages() {
      ...
      // 4. Превью встроенных и пользовательских шаблонов.
...
function renderLeftSummary() {
    ...
    function hydrateTemplatePreviewIcons(root = document.getElementById('stepContent')) {
      ...
      slots.forEach((slot) => {
        ...
        if (template && template.previewImage) {
          const normalizedUrl = normalizeExportImageUrl(template.previewImage);
          const cachedDataUrl = resolvedImageDataUrls.get(normalizedUrl);
          if (cachedDataUrl) {
            art.innerHTML = `<img src="${escapeAttr(cachedDataUrl)}" alt="${escapeAttr(template.name)}" class="template-choice-icon-preview-img" />`;
            slot.classList.remove('is-loading');
            slot.classList.add('is-ready');
            return;
          }
          const cachedPromise = exportImageDataUrlPromises.get(normalizedUrl);
          if (cachedPromise) {
            slot.classList.add('is-loading');
            slot.classList.remove('is-ready');
            pendingSlots.push({ slot, art, template, promise: cachedPromise });
            return;
          }
          art.innerHTML = `<img src="${escapeAttr(template.previewImage)}" alt="${escapeAttr(template.name)}" class="template-choice-icon-preview-img" />`;
          slot.classList.remove('is-loading');
          slot.classList.add('is-ready');
        } else {
      ...
      pendingSlots.forEach(({ slot, art, template, promise }) => {
        promise.then((dataUrl) => {
          ...
          art.innerHTML = `<img src="${escapeAttr(dataUrl || template.previewImage)}" alt="${escapeAttr(template.name)}" class="template-choice-icon-preview-img" />`;
          slot.classList.remove('is-loading');
          slot.classList.add('is-ready');
        });
      });
      ...
      if (!hideDesign && state.selectedCustom) {
        ...
        if (customTpl && customTpl.previewImage) {
          ...
          html = `
            ...
            >
              <img src="${escapeAttr(customTpl.previewImage)}" alt="preview" style="object-fit:contain; object-position:center center; width:100%; height:100%; pointer-events:none; user-select:none;">
            </div>
          `;
        }
    ...
    function pregenerateAllFlatPreviewsInBackground() {
      ...
      // Линейная очередь — не нагружаем CPU/GPU параллельным экспортом канвасов.
      let index = 0;
    ...
    async function initializeConfigurator() {
      try {
        ...
        // Прогрев flat-preview cache для всех вариантов изделия. Запускается
        // в фоне (idle callbacks) после первого render, чтобы не задерживать
        // показ конструктора, но к моменту первого клика по цвету/изделию
        // нужная сцена уже в кеше → переключение мгновенное.
        pregenerateAllFlatPreviewsInBackground();
      } finally {
        requestAnimationFrame(() => {
          setShellLoading(false);
