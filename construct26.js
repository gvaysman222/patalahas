...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    // Блокируем браузерный visual viewport pinch-zoom — иначе на мобиле
    // (Chrome Android, Safari iOS) пальцами two-finger gesture зумит ВСЮ
    // вкладку браузера, наш pointer-handler не получает событий, state.cropZoom
    // не обновляется. После text input layout reflow браузер сбрасывает свой
    // visual zoom → пользователь видит «zoom reset».
    //
    // touch-action: pan-y НЕ блокирует визуальный pinch-zoom (это отдельный
    // механизм браузера, не контролируется CSS).
    //
    // Корректный путь: либо <meta name=viewport user-scalable=no> (но мы не
    // контролируем head на Tilda production), либо JS preventDefault на:
    //  • iOS Safari: gesturestart/change/end (проприетарные события)
    //  • Chrome Android / др.: touchstart с touches.length > 1
