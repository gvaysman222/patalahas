...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function getAssignedCartItemNumber(orderId) {
      const key = normalizeTrackedOrderId(orderId) || normalizeTrackedOrderId(getFormattedOrderId(state.orderId));
      ...
      // Стартуем с максимума среди уже назначенных в сессии и уже лежащих в корзине
      ...
      const next = Math.max(maxAssigned, getMaxCartItemNumberFromCart()) + 1;
      cartItemNumberByOrderId.set(key, next);
      return next;
    }
    function buildTildaCartTitle(payload = getOrderPayload()) {
      const itemNumber = getAssignedCartItemNumber(payload.orderId);
      const parts = [
        payload.productName,
    ...
    async function syncTildaUploadFields(tildaForm) {
      ...
      await withPreservedTildaCartFormValues(async () => {
        ...
        if (upwidgetContainer && filesForTilda.length) {
          // ВАЖНО: не чистим виджет и не перезаливаем уже прикреплённые файлы.
          // Раньше clear + повторная заливка всего набора приводила к потере
          // оригинала предыдущего товара (крупный файл падал на повторной заливке).
          // Догружаем только те файлы, которых ещё нет в форме.
          const presentKeys = getTildaFormUploadedFileKeys(tildaForm);
          const filesToUpload = filesForTilda.filter((file) => file?.name && !presentKeys.has(getCartUploadFileKey(file.name)));
          const toUploadNames = filesToUpload.map((file) => file?.name || '(без имени)');
          try {
            window.__cckLog?.('cart_upload_attach', {
              orderId: state.orderId,
              trackedTotal: filesForTilda.length,
              alreadyPresent: filesForTilda.length - filesToUpload.length,
              toUpload: toUploadNames,
            });
          } catch (_) {}
          if (!filesToUpload.length) {
            console.log(`[constructor] Все файлы уже прикреплены к корзине Tilda (upwidget): ${filesForTilda.length} шт.`, fileNames);
            return;
          }
          const uploaded = await uploadFilesToTildaUpwidget(upwidgetContainer, filesToUpload, { appendExisting: true });
          if (uploaded) {
            console.log(`[constructor] Догружены файлы в корзину Tilda (upwidget): +${filesToUpload.length} (всего в наборе ${filesForTilda.length}).`, toUploadNames);
            // Контрольная сверка: убеждаемся, что в форме теперь есть весь набор.
            const stillMissing = getMissingTrackedCartUploadFiles(tildaForm);
            if (stillMissing.length) {
              try { window.__cckLog?.('cart_upload_attach_incomplete', { missing: stillMissing.map((f) => f?.name || '') }); } catch (_) {}
              console.warn('[constructor] После догрузки часть файлов всё ещё отсутствует, повторяю.', stillMissing.map((f) => f?.name || ''));
              await uploadFilesToTildaUpwidget(upwidgetContainer, stillMissing, { appendExisting: true }).catch(() => false);
            }
            return;
          }
          console.warn('[constructor] uploadFilesToTildaUpwidget вернул false, пробуем input[type=file] фолбэк.');
...
function getTildaCartForm() {
    ...
    // Дозаливаем в форму Tilda только те отслеживаемые файлы, которых там сейчас нет,
    // НЕ трогая уже прикреплённые. Нужно, потому что при добавлении второго товара
    // Tilda перерисовывает корзину и может выбросить ранее вставленные input'ы —
    // без восстановления фото первого/второго товара теряются.
    async function reconcileTrackedCartUploadFilesToTildaForm(tildaForm = findTildaOrderForm({ requireFileInput: true })) {
      if (!tildaForm) return false;
      const missingFiles = getMissingTrackedCartUploadFiles(tildaForm);
      if (!missingFiles.length) return false;
      const missingNames = missingFiles.map((file) => file?.name || '(без имени)');
      try { window.__cckLog?.('cart_upload_missing_detected', { missing: missingNames, trackedTotal: getCartUploadFilesSnapshot().length }); } catch (_) {}
      console.warn('[constructor] Обнаружены отсутствующие в форме файлы, восстанавливаем:', missingNames);
      const preservedFormValues = captureTildaCartFormValues(tildaForm);
      return withPreservedTildaCartFormValues(async () => {
        const upwidgetContainer = await waitForTildaUpwidgetContainer(tildaForm, 1500);
        if (upwidgetContainer) {
          const uploaded = await uploadFilesToTildaUpwidget(upwidgetContainer, missingFiles, { appendExisting: true }).catch(() => false);
          try { window.__cckLog?.('cart_upload_missing_restored', { missing: missingNames, uploaded: Boolean(uploaded), kind: 'upwidget' }); } catch (_) {}
          if (uploaded) {
            console.log(`[constructor] Восстановлены файлы в корзине Tilda (upwidget): ${missingFiles.length} шт.`, missingNames);
          } else {
            console.warn('[constructor] Не удалось восстановить файлы через upwidget.', missingNames);
          }
          return Boolean(uploaded);
        }
        const fileInputs = Array.from(tildaForm.querySelectorAll('.t-input-group_uw input[type="file"], input[type="file"]'));
        if (!fileInputs.length) {
          console.warn('[constructor] Нет upwidget/input[type=file] для восстановления файлов.', missingNames);
          return false;
        }
        // Для обычного input[type=file] докинуть один файл нельзя — переустанавливаем
        // весь отслеживаемый набор, чтобы попали все товары.
        const synced = syncFilesToAvailableInputs(fileInputs, getCartUploadFilesSnapshot());
        try { window.__cckLog?.('cart_upload_missing_restored', { missing: missingNames, uploaded: Boolean(synced), kind: 'file-input' }); } catch (_) {}
    ...
    async function syncCartUploadFilesWithActiveCart() {
      ...
      try {
        ...
        if (changed) {
          // Набор отслеживаемых файлов уменьшился (корзина очищена / товар удалён) —
          // делаем авторитетную пересинхронизацию (с очисткой лишнего).
          await syncTrackedCartUploadFilesToTildaForm(targetForm);
          try { window.__cckLog?.('cart_upload_synced_after_change', { activeCount, trackedTotal: getCartUploadFilesSnapshot().length }); } catch (_) {}
          return true;
        }
        // Набор не менялся, но Tilda могла выбросить наши файлы при перерисовке корзины —
        // восстанавливаем недостающие, не трогая уже прикреплённые.
        return await reconcileTrackedCartUploadFilesToTildaForm(targetForm);
      } finally {
