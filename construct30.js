...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    // Захват браузерного native pinch-zoom через visualViewport API.
    // Когда юзер на мобиле зумит страницу двумя пальцами (browser visual
    // viewport), наш pointer-handler на garment-wrap может не сработать
    // (Tilda CSS, layout, focus). Этот хендлер берёт visualViewport.scale
    // и переводит его в state.cropZoom — фото в превью увеличивается, а
    // потом, когда браузер сбрасывает свой visual zoom (после ввода текста /
    // появления клавиатуры), наш state.cropZoom остаётся (защищён setter'ом),
    // и render показывает фото в правильном размере.
    if (typeof window !== 'undefined' && window.visualViewport) {
      const vv = window.visualViewport;
      let __vvLastAppliedScale = 1;
      const handleVvScale = () => {
        const scale = Number(vv.scale) || 1;
        // Только когда юзер реально зумит и есть активная загрузка фото.
        const hasUpload = Array.isArray(state.uploads) && state.uploads.length > 0;
        if (!hasUpload) {
          __vvLastAppliedScale = scale;
          return;
        }
        // Применяем только значимое изменение (> 0.02), чтобы не дёргать рендер
        // на каждый пиксель резинки.
        if (Math.abs(scale - __vvLastAppliedScale) < 0.02) return;
        __vvLastAppliedScale = scale;
        // visualViewport.scale = 1.5 → photo zoom = 1.5, clamp 1..3.
        const target = Math.max(1, Math.min(3, scale));
        if (Math.abs(target - state.cropZoom) > 0.02) {
          state.cropZoom = target;
          if (typeof renderVisualizer === 'function') {
            try { renderVisualizer(); } catch (_) {}
          }
        }
      };
      vv.addEventListener('resize', handleVvScale);
      vv.addEventListener('scroll', handleVvScale);
    }
