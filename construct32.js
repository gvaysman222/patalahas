...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    // === Custom Preview Pinch Zoom + verbose debug ===
    // Дебажим всё: каждое touchstart/move/end логируется в trace.
    if (typeof window !== 'undefined' && typeof document !== 'undefined') {
      let pinchStartDistance = 0;
      let pinchStartScale = 1;
      let pinchTargetWrap = null;
      let __previewVisualScale = 1;
      window.__previewVisualScaleGet = () => __previewVisualScale;
      const __pinchTrace = [];
      window.__pinchTrace = __pinchTrace;
      const logPinch = (kind, info) => {
        __pinchTrace.push({ t: new Date().toISOString().slice(11,19), kind, ...info });
        if (__pinchTrace.length > 30) __pinchTrace.shift();
        // Также покажу в той же debug-панели
        const dbg = document.getElementById('cropDebugContent');
        if (dbg) {
          const baseLines = dbg.textContent.split('\n').slice(0, 40);
          const pinchLines = __pinchTrace.slice(-10).map(t =>
            `[PINCH] ${t.t} ${t.kind} ${JSON.stringify(info).slice(0, 60)}`
          );
          dbg.textContent = baseLines.join('\n') + '\n----\n' + pinchLines.join('\n')
            + '\n----\nvisualScale=' + __previewVisualScale.toFixed(2);
        }
      };
      ...
      const applyVisualScale = (scale) => {
        ...
        if (!wrap) { logPinch('apply-fail', { reason: 'no wrap' }); return; }
        __previewVisualScale = Math.max(1, Math.min(3, scale));
        wrap.style.transform = `scale(${__previewVisualScale})`;
        wrap.style.transformOrigin = '50% 50%';
      };
      window.addEventListener('touchstart', (e) => {
        const tCount = e.touches.length;
        logPinch('touchstart', { touches: tCount });
        if (tCount === 2) {
          e.preventDefault();
          pinchStartDistance = getDistance(e.touches);
          pinchStartScale = __previewVisualScale;
          pinchTargetWrap = document.getElementById('garmentWrap');
          logPinch('pinch-start', { dist: Math.round(pinchStartDistance), startScale: pinchStartScale });
        }
      ...
      window.addEventListener('touchmove', (e) => {
        if (e.touches.length === 2 && pinchStartDistance > 0) {
          ...
          // logPinch reduced (every move flood)
        }
      }, { passive: false, capture: true });
      window.addEventListener('touchend', (e) => {
        if (e.touches.length < 2 && pinchStartDistance > 0) {
          logPinch('pinch-end', { finalScale: __previewVisualScale });
