...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function pruneCartUploadFilesByActiveCart(products = window.tcart?.products) {
      ...
      console.log('[CCK-DEL] prune: активные номера в корзине =', Array.from(activeNumbers), '| activeCount =', activeCount,
      ...
      console.log(`[CCK-DEL] prune результат: удалено файлов=${beforeFiles - cartUploadFiles.length}, удалено описаний=${removedOrderIds.length}, осталось файлов=${cartUploadFiles.length}, changed=${changed}`);
      if (changed) {
...
function getTildaCartForm() {
    ...
    async function syncCartUploadFilesWithActiveCart() {
      if (cartUploadCleanupInFlight) {
        console.log('[CCK-DEL] cleanup пропущен — уже выполняется.');
        return false;
      }
      cartUploadCleanupInFlight = true;
      try {
        console.log('[CCK-DEL] cleanup START (синхронизация состава корзины).');
        ...
        console.log(`[CCK-DEL] cleanup: changed=${changed}, файлов в наборе=${trackedFiles.length}, в виджете=${widgetKeysNow.length}, не хватает=${missingCount}, лишних(stale)=${hasStaleInWidget}`,
          '| виджет:', widgetKeysNow, '| набор:', Array.from(trackedKeys));
        if (!changed && !missingCount && !hasStaleInWidget) {
          console.log('[CCK-DEL] cleanup: всё совпадает, ничего не делаю.');
          return false;
        }
    ...
    function scheduleCartUploadCleanup(delayMs = 260) {
      ...
      cartUploadCleanupTimer = window.setTimeout(() => {
        if (orderSubmitInFlight) {
          console.log('[CCK-DEL] проверку отложил — идёт оформление (orderSubmitInFlight).');
    ...
    function bindCartCleanupClickTrigger() {
      ...
      document.addEventListener('click', (event) => {
        ...
        console.log('[CCK-DEL] Клик в корзине Tilda — планирую перепроверку состава (300/1000/2500 мс).');
    ...
    function ensureCartUploadCleanupObserver(cartWindow = getTildaCartWindow()) {
      ...
      console.log('[CCK-DEL] Наблюдатель за корзиной подключён к', observeTarget?.className || observeTarget?.nodeName);
