...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    function setupPreviewZoom() {
      ...
      const applyTransform = (animate) => {
        garmentWrap.style.transition = animate ? 'transform .25s ease' : 'none';
        if (currentZoom <= 1) {
          garmentWrap.style.transform = '';
          garmentWrap.style.transformOrigin = '50% 50%';
        } else {
          garmentWrap.style.transform = `translate(${currentTx}px, ${currentTy}px) scale(${currentZoom})`;
        }
      };
      ...
      if (isDesktop) {
        ...
        const applyDesktopZoom = (e, animate) => {
          const parent = garmentWrap.parentElement;
          const rect = parent.getBoundingClientRect();
          const fx = (e.clientX - rect.left) / rect.width - 0.5;
          const fy = (e.clientY - rect.top) / rect.height - 0.5;
          currentTx = -fx * (DESKTOP_ZOOM - 1) * rect.width;
          currentTy = -fy * (DESKTOP_ZOOM - 1) * rect.height;
          currentZoom = DESKTOP_ZOOM;
          garmentWrap.style.transition = animate ? 'transform .2s ease' : 'none';
          garmentWrap.style.transform = `translate(${currentTx}px, ${currentTy}px) scale(${DESKTOP_ZOOM})`;
        };

        garmentWrap.addEventListener('mouseenter', (e) => {
          applyDesktopZoom(e, true);
        }, { signal });

        garmentWrap.addEventListener('mousemove', (e) => {
          if (currentZoom <= 1) return;
          applyDesktopZoom(e, false);
        }, { signal });
    ...
    // Защита zoom от незапланированных сбросов + ВИДИМЫЙ DEBUG-OVERLAY
    // (right-top угол) — показывает каждое изменение state.cropZoom с
    // источником (callsite). На реальном устройстве можно скриншотить
    // и посылать разработчику.
    let __cropZoomValue = state.cropZoom;
    let __cropZoomAllowReset = false;
    const __cropTrace = [];
    let __cropTraceCounter = 0;
    Object.defineProperty(state, 'cropZoom', {
      ...
      set(next) {
        ...
        // Парсим stack — берём 1-2 значимых кадра (file:line:col).
        let caller = '';
        try {
          const stack = (new Error()).stack || '';
          const lines = stack.split('\n').slice(1, 8);
          // Ищем строку с конструктором (исключая defineProperty / setter)
          const interesting = lines.find((l) => l.includes('.js') && !l.includes('Object.set')) || lines[1] || '';
          const m = interesting.match(/at\s+(\S+)\s+.+:(\d+):\d+/) || interesting.match(/(\S+\.js):(\d+):/);
          caller = m ? `${m[1] || ''}:${m[2] || ''}` : interesting.slice(0, 80);
        } catch (_) {}
        __cropTraceCounter += 1;
        __cropTrace.push({
          n: __cropTraceCounter,
          time: new Date().toISOString().slice(11, 19),
          from: __cropZoomValue.toFixed(2),
          to: numericNext.toFixed(2),
          blocked,
          caller: caller || '(unknown)',
        });
        if (__cropTrace.length > 30) __cropTrace.shift();
        if (typeof window !== 'undefined') window.__cropTrace = __cropTrace;
        if (blocked) {
          __renderCropDebug();
          return;
        }
    ...
    let __cropDebugEl = null;
    function __renderCropDebug() {
      if (typeof document === 'undefined') return;
      if (!__cropDebugEl || !__cropDebugEl.isConnected) {
        if (!document.body) return;
        __cropDebugEl = document.createElement('div');
        __cropDebugEl.id = 'cropDebugBox';
        __cropDebugEl.style.cssText = [
          'position:fixed','top:6px','right:6px','z-index:2147483647',
          'background:rgba(0,0,0,0.85)','color:#0f0',
          'font:10px/1.25 monospace','padding:6px 8px',
          'border-radius:6px','max-width:240px','max-height:50vh',
          'overflow:auto','pointer-events:auto','user-select:text',
          'white-space:pre-wrap','word-break:break-all',
        ].join(';');
        // Кнопка Clear
        const clearBtn = document.createElement('button');
        clearBtn.textContent = 'X';
        clearBtn.style.cssText = 'position:absolute;top:2px;right:4px;background:#400;color:#fff;border:0;padding:1px 4px;font:9px monospace;border-radius:3px;cursor:pointer';
        clearBtn.onclick = (e) => { e.stopPropagation(); __cropTrace.length = 0; __renderCropDebug(); };
        __cropDebugEl.appendChild(clearBtn);
        const content = document.createElement('div');
        content.id = 'cropDebugContent';
        content.style.marginTop = '14px';
        __cropDebugEl.appendChild(content);
        document.body.appendChild(__cropDebugEl);
      }
      const content = __cropDebugEl.querySelector('#cropDebugContent');
      if (!content) return;
      const lines = [
        `zoom = ${__cropZoomValue.toFixed(2)}  (changes: ${__cropTraceCounter})`,
        '----',
        ...__cropTrace.slice(-20).map((t) => (
          `${t.n}. ${t.time} ${t.from}→${t.to}${t.blocked ? ' [BLOCKED]' : ''}\n   ${t.caller}`
        )),
      ];
      content.textContent = lines.join('\n');
      content.scrollTop = content.scrollHeight;
    }
    // Показать пустую панель сразу после загрузки body.
    if (typeof document !== 'undefined') {
      const tryInit = () => {
        if (document.body) __renderCropDebug();
        else requestAnimationFrame(tryInit);
      };
      tryInit();
    }

    // === Custom Preview Pinch Zoom ===
    // Юзер ожидает что pinch на step 4 = увеличить ВЕСЬ блок превью
    // (визуально как если бы зумил страницу). state.cropZoom — это другое
    // (зум фото внутри рамки на футболке). Browser native viewport zoom
    // сбрасывается при появлении клавиатуры (text input).
    //
    // Решение: ловим touch-события на capture-фазе (раньше любых Tilda
    // обработчиков). Если 2 пальца — это pinch. Применяем CSS-transform
    // scale() напрямую на .garment-wrap. Block browser native pinch.
    // CSS scale остаётся при text input — никакого «zoom reset».
    if (typeof window !== 'undefined' && typeof document !== 'undefined') {
      let pinchStartDistance = 0;
      let pinchStartScale = 1;
      let pinchTargetWrap = null;
      let __previewVisualScale = 1; // current scale of garment-wrap
      const getDistance = (touches) => {
        const a = touches[0], b = touches[1];
        return Math.hypot(b.clientX - a.clientX, b.clientY - a.clientY);
      };
      const applyVisualScale = (scale) => {
        const wrap = document.getElementById('garmentWrap');
        if (!wrap) return;
        __previewVisualScale = Math.max(1, Math.min(3, scale));
        wrap.style.transform = `scale(${__previewVisualScale})`;
        wrap.style.transformOrigin = '50% 50%';
      };
      // На touchstart с 2 пальцами — начинаем pinch и блокируем browser zoom.
      window.addEventListener('touchstart', (e) => {
        if (e.touches.length === 2) {
          // Только если касание попадает в preview-area (или близко к ней)
          const wrap = document.getElementById('garmentWrap');
          if (!wrap) return;
          const wrapRect = wrap.getBoundingClientRect();
          const t = e.touches[0];
          // Проверяем что хотя бы один палец в области превью или рядом
          const inPreview = t.clientX >= wrapRect.left - 100 && t.clientX <= wrapRect.right + 100
            && t.clientY >= wrapRect.top - 100 && t.clientY <= wrapRect.bottom + 100;
          if (!inPreview) return;
          e.preventDefault();
          pinchTargetWrap = wrap;
          pinchStartDistance = getDistance(e.touches);
          pinchStartScale = __previewVisualScale;
        }
      }, { passive: false, capture: true });
      window.addEventListener('touchmove', (e) => {
        if (e.touches.length === 2 && pinchTargetWrap && pinchStartDistance > 0) {
          e.preventDefault();
          const currentDistance = getDistance(e.touches);
          const factor = currentDistance / pinchStartDistance;
          applyVisualScale(pinchStartScale * factor);
        }
      }, { passive: false, capture: true });
      window.addEventListener('touchend', (e) => {
        if (e.touches.length < 2) {
          pinchStartDistance = 0;
          pinchTargetWrap = null;
        }
      }, { passive: false, capture: true });
      // Double-tap → reset visual scale (UX дать возможность вернуть в обычный вид)
      let __lastTap = 0;
      window.addEventListener('touchend', (e) => {
        const now = Date.now();
        if (now - __lastTap < 300 && e.touches.length === 0) {
          const wrap = document.getElementById('garmentWrap');
          if (!wrap) return;
          const wrapRect = wrap.getBoundingClientRect();
          const t = e.changedTouches[0];
          if (t && t.clientX >= wrapRect.left && t.clientX <= wrapRect.right
            && t.clientY >= wrapRect.top && t.clientY <= wrapRect.bottom) {
            applyVisualScale(__previewVisualScale > 1.05 ? 1 : 2);
          }
        }
        __lastTap = now;
      }, { passive: true });
    ...
    // Блокируем браузерный visual viewport pinch-zoom несколькими способами,
    // потому что разные браузеры реагируют по-разному:
    //
    // 1. <meta viewport user-scalable=no, maximum-scale=1> — самый надёжный,
    //    проверяется на уровне compositor (до JS). Динамически инжектируем
    //    или модифицируем существующий тег.
    // 2. JS preventDefault на gesturestart/change/end (iOS Safari) и
    //    touchstart/touchmove с >1 пальцем (Chrome Android и др.).
    // 3. CSS touch-action: pan-y на body — дополнительная страховка.
    //
    // touch-action: pan-y САМ ПО СЕБЕ НЕ блокирует визуальный pinch-zoom
    // (это отдельный механизм браузера, controlled только meta viewport).
    if (typeof document !== 'undefined') {
      // Inject / patch <meta name="viewport"> — самое надёжное.
      try {
        ...
        const desiredContent = 'width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no';
        if (metaViewport.getAttribute('content') !== desiredContent) {
    ...
    if (typeof document !== 'undefined') {
      const blockMultiTouch = (e) => {
        try {
          // iOS Safari GestureEvent — всегда блокируем
          if (e.type === 'gesturestart' || e.type === 'gesturechange' || e.type === 'gestureend') {
            e.preventDefault();
            return;
          }
          // Touch event с >1 пальцем = pinch жест → блокируем default
      ...
      ['gesturestart', 'gesturechange', 'gestureend'].forEach((name) => {
        document.addEventListener(name, blockMultiTouch, { passive: false });
      });
      ['touchstart', 'touchmove'].forEach((name) => {
        document.addEventListener(name, blockMultiTouch, { passive: false });
      });
      // Дополнительно: запрещаем double-tap-to-zoom (это другой механизм).
      ...
      if (state.step === 4) {
        ...
        if (textInput) {
          ...
          textInput.oninput = (e) => {
            ...
            // Защита от сброса зума/позиции на мобиле: после ввода текста
            // updateStepFourControls вызывает normalizeTextCustomizationState
            // / re-render, которые в некоторых сценариях могут сбросить
            // state.cropZoom (через persisted snapshot reload, render() из
            // font-list mismatch и т.д.). Сохраняем/восстанавливаем явно.
...
function renderLeftSummary() {
    ...
    function renderVisualizer() {
      ...
      if (isKonvaAvailable()) {
        ...
        if (KONVA_DEV_MODE) {
          // Dev mode: live Konva canvas overlay (draggable elements)
          ...
          // Show garment base image (Konva draws overlay only)
