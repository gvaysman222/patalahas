...
function getTildaCartForm() {
    ...
    async function syncCartUploadFilesWithActiveCart() {
      ...
      try {
        ...
        const hasStaleInWidget = Array.from(getTildaFormUploadedFileKeys(targetForm)).some((widgetKey) => !trackedKeys.has(widgetKey));
        if (!changed && !missingCount && !hasStaleInWidget) return false;
