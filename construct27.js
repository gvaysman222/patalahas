...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    // Защита zoom от незапланированных сбросов: переопределяем свойство
    // state.cropZoom так, чтобы переход >1 → 1 был запрещён, кроме случаев
    // явных user-actions (file upload, file remove, явный slider input на 1).
    // Любой невидимый ре-render (text change, color change, normalize-state)
    // не сможет случайно сбросить zoom.
    ...
    Object.defineProperty(state, 'cropZoom', {
      ...
      set(next) {
        ...
        // Если текущий zoom > 1.01 и хотят сбросить в ≤ 1.01 без явного
        // разрешения — игнорируем. Это спасает от любого невидимого reset,
        // приходящего через persistence reload, normalize, async race и т.д.
        if (
