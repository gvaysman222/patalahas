...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    // Защита zoom от незапланированных сбросов + отладочный трейс.
    let __cropZoomValue = state.cropZoom;
    let __cropZoomAllowReset = false;
    if (typeof window !== 'undefined') {
      window.__cropTrace = [];
      window.__cropZoomGetCurrent = () => __cropZoomValue;
    }
    Object.defineProperty(state, 'cropZoom', {
      ...
      set(next) {
        ...
        if (typeof window !== 'undefined' && window.__cropTrace) {
          window.__cropTrace.push({
            t: Date.now(),
            from: __cropZoomValue,
            to: numericNext,
            blocked,
            allow: __cropZoomAllowReset,
            caller: callerLine,
          });
          if (window.__cropTrace.length > 50) window.__cropTrace.shift();
        }
        if (blocked) return;
