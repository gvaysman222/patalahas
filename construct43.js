...
function getTildaCartForm() {
    ...
    async function ensureCartFileUploadedToCdn(container, file) {
      ...
      const priorAttempts = cdnUploadAttemptsByKey.get(key) || 0;
      // Страховка от бесконечных повторов на действительно «битом» файле.
      if (priorAttempts >= 9) {
        console.warn(`[CCK-UPLOAD] Файл превысил лимит попыток заливки (${priorAttempts}), пропускаю: ${file.name}`);
        return null;
      }
      ...
      for (let attempt = 1; attempt <= 3 && !uploaded; attempt += 1) {
        ...
        try {
          uploaded = await uploadFileToTildaUpwidgetApi(container, file);
        } catch (error) {
          try { window.__cckLog?.('cart_upload_api_retry', { file: file.name, attempt: priorAttempts + attempt, error: String(error && error.message || error) }); } catch (_) {}
          console.warn(`[CCK-UPLOAD] Ошибка заливки ${file.name} (попытка ${priorAttempts + attempt}), повторю.`, error);
          if (attempt < 3) await delay(300 * attempt);
        }
      }
      if (uploaded) {
      ...
      console.error(`[CCK-UPLOAD] ✗ Файл НЕ загружен после попыток: ${file.name}`);
      return null;
    }
