...
// ===== constructor-checkout-mvp-tilda-logger.html =====
// Self-contained client logger. Reports structured user actions to
// https://logs.zgram.pro/ingest. Designed to never throw and never block the
// UI thread. If the endpoint is unreachable the queue is silently dropped.
//
// Identification:
//   - clientId: UUID stored in localStorage (survives across sessions).
//   - sessionId: UUID generated on every page load (per tab).
//   - orderId / step / contacts: attached by point-level callers via
//     window.__cckLog('event', data).
//
// Surface:
//   - window.__cckLog(event, data)     -> enqueue a structured event
//   - window.__cckLogger.flush()       -> manual flush (debug)
//   - window.__cckLogger.clientId      -> current client UUID
//   - window.__cckLogger.sessionId     -> current session UUID
//
// Network: text/plain POST so the request is a CORS "simple request" without
// preflight. Server (logs.zgram.pro/ingest) parses text/plain as JSON.
(function initConstructorLogger(global) {
  if (global.__cckLogger) return; // script included twice — be idempotent

  const ENDPOINT = 'https://logs.zgram.pro/ingest';
  const FLUSH_INTERVAL_MS = 3000;
  const FLUSH_BATCH = 25;
  const MAX_QUEUE = 200; // drop oldest beyond this
  ...
  function readClientId() {
    try {
      const KEY = 'cck_log_cid';
      let id = global.localStorage && global.localStorage.getItem(KEY);
      if (!id) {
        id = uuid();
        try { global.localStorage.setItem(KEY, id); } catch (_) {}
      }
      return id;
    } catch (_) {
      return uuid();
  ...
  const queue = [];
  // Running snapshot of the user's most recent choices. Updated by the
  // delegated click listener so later events carry the latest context.
  const lastCtx = {
    product: null, color: null, size: null, seam: null,
    template: null, custom: null, font: null, textColor: null,
    stepTwoTab: null,
  };
  let dropped = 0;
  ...
  function ctxSnapshot() {
    return {
      product: lastCtx.product, color: lastCtx.color, size: lastCtx.size,
      seam: lastCtx.seam, template: lastCtx.template, custom: lastCtx.custom,
      font: lastCtx.font, textColor: lastCtx.textColor,
      stepTwoTab: lastCtx.stepTwoTab,
    };
  }
  ...
  function flush(useBeacon) {
    ...
    if (dropped) {
      batch.push({
        clientId, sessionId, pageLoadAt,
        ts: new Date().toISOString(),
        event: 'logger_dropped',
        data: { dropped },
      });
      dropped = 0;
    }
  ...
  if (global.addEventListener) {
    ...
    global.addEventListener('beforeunload', () => flush(true));
    ...
    global.addEventListener('unhandledrejection', (e) => {
      enqueue('js_unhandled_rejection', {
        reason: String((e && e.reason && (e.reason.message || e.reason)) || ''),
      });
    });
  }

  // Delegated DOM watchers ---------------------------------------------------
  // Captures clicks on data-* options without modifying individual handlers.
  // We use capture: true so the event is recorded even if a downstream handler
  // calls stopPropagation.
  const dataAttrMap = {
    'data-product':      ['option_product',     'product'],
    'data-color':        ['option_color',       'color'],
    'data-size':         ['option_size',        'size'],
    'data-seam':         ['option_seam',        'seam'],
    'data-template':     ['design_template',    'template'],
    'data-custom':       ['design_custom',      'custom'],
    'data-font-value':   ['option_font',        'font'],
    'data-text-color':   ['option_text_color',  'textColor'],
    'data-step-two-tab': ['step2_tab_switched', 'stepTwoTab'],
  };
  ...
  function onClick(e) {
    try {
      ...
      for (const attr in dataAttrMap) {
        const el = target.closest && target.closest('[' + attr + ']');
        if (!el) continue;
        const value = el.getAttribute(attr);
        const meta = dataAttrMap[attr];
        const prev = lastCtx[meta[1]];
        lastCtx[meta[1]] = value;
        enqueue(meta[0], { value, prev });
        return;
      }
      const phrase = target.closest && target.closest('[data-phrase]');
      if (phrase) { enqueue('text_phrase_inserted', { phrase: phrase.getAttribute('data-phrase') }); return; }
      const edit = target.closest && target.closest('[data-edit-upload]');
      if (edit) { enqueue('file_edit_opened', { index: Number(edit.getAttribute('data-edit-upload')) }); return; }
      const remove = target.closest && target.closest('[data-remove-upload]');
      if (remove) { enqueue('file_remove_clicked', { index: Number(remove.getAttribute('data-remove-upload')) }); return; }
      const submit = target.closest && target.closest('[data-submit-order]');
      if (submit) { enqueue('order_submit_clicked', {}); return; }
      if (target.closest && target.closest('#nextBtn')) { enqueue('step_next_clicked', {}); return; }
      if (target.closest && target.closest('#prevBtn')) { enqueue('step_prev_clicked', {}); return; }
    } catch (_) {}
  }
  ...
  function onChange(e) {
    try {
      ...
      if (t.id === 'zoom') {
        enqueue('crop_zoom_changed', { value: Number(t.value) });
        return;
      }
      if (t.type === 'file') {
        const files = t.files ? Array.from(t.files) : [];
        enqueue('file_input_change', {
          count: files.length,
  ...
  // Text input is debounced (1.2s of inactivity) so we capture intent, not
  // every keystroke. Field name is logged; the value itself is NOT — only
  // length, to avoid leaking customer PII or design text on the server.
  let textInputTimer = null;
  let lastTextSnapshot = null;
  function onInput(e) {
    try {
      const t = e.target;
      if (!t || t.nodeType !== 1) return;
      const isContact = ['configuratorName','configuratorEmail','configuratorPhone','configuratorComment'].includes(t.id);
      const isText = t.id === 'configuratorText' || (t.matches && t.matches('[data-configurator-text]'));
      if (!isContact && !isText) return;
      clearTimeout(textInputTimer);
      lastTextSnapshot = {
        field: t.id || 'configuratorText',
        length: (t.value || '').length,
      };
      textInputTimer = setTimeout(() => {
        if (lastTextSnapshot) enqueue('field_typed', lastTextSnapshot);
  ...
  if (global.document && global.document.readyState !== 'loading') {
    ...
    global.document.addEventListener('DOMContentLoaded', bindDom);
  }

  global.__cckLog = function (event, data) { enqueue(event, data); };
  global.__cckLogger = { flush, queue, clientId, sessionId };

  enqueue('page_loaded', {
    ua: global.navigator ? global.navigator.userAgent : null,
    referrer: global.document ? global.document.referrer : null,
    screen: global.screen ? { w: global.screen.width, h: global.screen.height } : null,
    viewport: { w: global.innerWidth || 0, h: global.innerHeight || 0 },
  });
...
(function initConstructorCheckoutTemplateConfig(global) {
  ...
  const centerEmbroideryFonts = [
    ...
    },
    {
      ...
      lineHeightScale: 0.5,
      fontWeight: 400,
      lowercasePreviewScale: 1.35,
      lowercaseExportScale: 1.35,
      lowercaseLineHeightScale: 0.5,
      lowercaseFontWeight: 400,
    },
  ];
  const centerPrintFonts = [
    ...
    },
    {
      ...
      lineHeightScale: 0.35,
      lowercasePreviewScale: 1.64,
      lowercaseExportScale: 1.90,
      lowercaseLineHeightScale: 0.35,
      lowercaseFontWeight: 400,
    },
    {
  ...
  global.CONSTRUCTOR_CHECKOUT_TEMPLATE_CONFIG = {
    templates: [
      { id: 'emb-center', name: 'Текст по центру', uploads: 0, kind: 'emb-center', price: 650, previewImage: 'https://optim.tildacdn.com/tild3639-3163-4730-a265-376630656335/-/format/webp/_1.jpg' },
      { id: 'emb-left', name: 'Текст слева', uploads: 0, kind: 'emb-left', price: 650, previewImage: 'https://optim.tildacdn.com/tild3465-3566-4166-a338-306335396234/-/format/webp/_2.jpg' },
      { id: 'simple-photo', name: 'Просто фото', uploads: 1, kind: 'simple-photo', price: 5, prices: { s: 5, l: 5 }, previewImage: 'https://optim.tildacdn.com/tild3364-6539-4862-a532-316231366134/-/resize/150x/-/format/webp/_3.jpg' }, // TEST_PRICE: было price 900 / prices {s:900,l:1200}, вернуть
      { id: 'photo-top-text', name: 'Фото + надпись сверху', uploads: 1, kind: 'photo-top-text', price: 1200, prices: { s: 1200, l: 1500 }, previewImage: 'https://optim.tildacdn.com/tild3330-6263-4561-b637-346231303632/-/format/webp/_4.jpg' },
      { id: 'photo-bottom-text', name: 'Фото + надпись снизу', uploads: 1, kind: 'photo-bottom-text', price: 1200, prices: { s: 1200, l: 1500 }, previewImage: 'https://optim.tildacdn.com/tild3938-3263-4830-a266-396363323735/-/format/webp/_5.jpg' },
      { id: 'photo-diagonal-text', name: 'Фото + надпись по диагонали', uploads: 1, kind: 'photo-diagonal-text', price: 1200, prices: { s: 1200, l: 1500 }, previewImage: 'https://optim.tildacdn.com/tild3833-3433-4937-a537-343831643166/-/format/webp/_6.jpg' },
      { id: 'polaroid-photo', name: 'Полароид', uploads: 1, kind: 'polaroid-photo', price: 900, prices: { s: 900, l: 1200 }, previewImage: 'https://optim.tildacdn.com/tild6666-6336-4465-b237-366666633337/-/format/webp/_7.jpg' },
      { id: 'polaroid-top-text', name: 'Полароид + надпись сверху', uploads: 1, kind: 'polaroid-top-text', price: 1200, prices: { s: 1200, l: 1500 }, previewImage: 'https://optim.tildacdn.com/tild3439-3632-4163-a335-396438633733/-/resize/150x/-/format/webp/_8.jpg' },
      { id: 'polaroid-diagonal-text', name: 'Полароид + надпись по диагонали', uploads: 1, kind: 'polaroid-diagonal-text', price: 1200, prices: { s: 1200, l: 1500 }, previewImage: 'https://optim.tildacdn.com/tild3235-3166-4362-a135-633036626632/-/format/webp/_9.jpg' },
      { id: 'polaroid-inside-text', name: 'Полароид + текст внутри', uploads: 1, kind: 'polaroid-inside-text', price: 900, prices: { s: 900, l: 1200 }, previewImage: 'https://optim.tildacdn.com/tild3630-3666-4466-a238-396234643732/-/format/webp/_10.jpg' },
    ],
    customTemplates: [
      { id: 'custom-1', name: 'Парный полароид', note: 'Разработка дизайнером', price: 1300, previewImage: 'https://www.dropbox.com/scl/fo/32sdavmje7anx9fg8h13a/AJvA4PbLVJpbcO4OhhvXLUQ/%D0%BF%D0%BE%D0%BB%D0%B0%D1%80%D0%BE%D0%B8%D0%B4%20%D0%B4%D0%B2%D0%BE%D0%B9%D0%BD%D0%BE%D0%B9%20%D0%BD%D0%BE%D0%B2%D1%8B%D0%B9.png?rlkey=hko8iad8atbm6bukwynhqlvps&st=8ydu7ovl&raw=1', cardImage: 'https://optim.tildacdn.com/tild3261-3761-4434-b836-346635626363/-/format/webp/_01.jpg', photoDefaults: { exportX: 407, exportY: 481.1, width: 192.1, height: 128.8, scale: 2.183 } },
      { id: 'custom-2', name: 'Силуэт', note: 'Разработка дизайнером', price: 1550, previewImage: 'https://www.dropbox.com/scl/fo/32sdavmje7anx9fg8h13a/APT4Bvd9uJzDm2uWBDiqHmk/002.png?rlkey=hko8iad8atbm6bukwynhqlvps&st=3sdpmecg&raw=1', cardImage: 'https://optim.tildacdn.com/tild6666-3465-4236-a566-356535643237/-/resize/212x/-/format/webp/_02.jpg', photoDefaults: { exportX: 431, exportY: 479.7, width: 155.4, height: 171.2, scale: 1.766 } },
      { id: 'custom-3', name: 'Голова в колпаке + фраза', note: 'Разработка дизайнером', price: 1350, previewImage: 'https://www.dropbox.com/scl/fo/32sdavmje7anx9fg8h13a/AKzgf8MyTITN8tOBkPRvLEk/003.png?rlkey=hko8iad8atbm6bukwynhqlvps&st=1qcfqkq1&raw=1', cardImage: 'https://optim.tildacdn.com/tild3466-6164-4939-b637-336439333361/-/format/webp/_03.jpg', photoDefaults: { exportX: 427.3, exportY: 456, width: 192.3, height: 206, scale: 2.185 } },
      { id: 'custom-4', name: 'Фраза из фото', note: 'Разработка дизайнером', price: 1350, previewImage: 'https://www.dropbox.com/scl/fo/32sdavmje7anx9fg8h13a/AH_cKJkhh_G52dcEwLxF2PA/004.png?rlkey=hko8iad8atbm6bukwynhqlvps&st=k7ryeajv&raw=1', cardImage: 'https://optim.tildacdn.com/tild6164-6438-4030-a133-663935663864/-/format/webp/_04.jpg', photoDefaults: { exportX: 373.5, exportY: 481.7, width: 275.5, height: 85.6, scale: 3.131 } },
      { id: 'custom-5', name: 'Коллаж из 3-х фото', note: 'Разработка дизайнером', price: 1700, previewImage: 'https://www.dropbox.com/scl/fo/32sdavmje7anx9fg8h13a/ALmdTR-0Axc6r3oD1b9iMt8/5.%20%D0%9A%D0%BE%D0%BB%D0%BB%D0%B0%D0%B6%20%D0%B8%D0%B7%203-%D1%85%20%D1%84%D0%BE%D1%82%D0%BE.png?rlkey=hko8iad8atbm6bukwynhqlvps&st=eviyphsl&raw=1', cardImage: 'https://optim.tildacdn.com/tild6331-3830-4634-b133-316439346331/-/format/webp/_05.jpg', photoDefaults: { exportX: 373.2, exportY: 480.2, width: 262.9, height: 102.2, scale: 2.987 } },
      { id: 'custom-6', name: 'Коллаж из 4-х фото', note: 'Разработка дизайнером', price: 2100, previewImage: 'https://www.dropbox.com/scl/fo/32sdavmje7anx9fg8h13a/ADU0wBCowM9BTqActIBrbzk/006.png?rlkey=hko8iad8atbm6bukwynhqlvps&st=fk9c1ez0&raw=1', cardImage: 'https://optim.tildacdn.com/tild6463-3736-4562-b938-343634343333/-/format/webp/_06.jpg', photoDefaults: { exportX: 423.2, exportY: 480, width: 168, height: 227.2, scale: 2.058 } },
    ],
    ...
    motivationalPhrases: [
      'маленькая ты гордилась бы собой',
      'маленький ты гордился бы собой',
      'у тебя всё получится',
      'when I grow up',
      'у тебя есть миллион причин гордиться собой',
      'don\'t worry, be happy',
      'the detstvo',
      'сияй!',
      'маленький человек с большими мечтами',
      'born to be brave and happy',
    ],
...
const baseProducts = [
      { id: 'tee-iznanka', name: 'Футболка [ИЗНАНКА]', price: 5, priceNote: 'Швы наружу' }, // TEST_PRICE: было 3400, вернуть
      { id: 'tee-base', name: 'Футболка [БАЗА]', price: 5, priceNote: 'Классические швы' }, // TEST_PRICE: было 3400, вернуть
      { id: 'longsleeve-choice', name: 'Лонгслив [ИЗНАНКА]', price: 3900, priceNote: 'Швы наружу' },
      { id: 'hoodie-basic', name: 'Худи [БАЗА]', price: 6400 },
      { id: 'sweatshirt-iznanka', name: 'Свитшот [ИЗНАНКА]', price: 5700 },
    ];
    const productImages = {
      'tee-iznanka': [
        {
          name: 'Белый',
          image: 'https://optim.tildacdn.com/tild3038-6534-4063-b332-623562326339/-/format/webp/ti_izn_white_white.jpg',
          seams: [
            { name: 'Белый' },
            { name: 'Олива', image: 'https://optim.tildacdn.com/tild6563-3561-4530-b938-373937336364/-/format/webp/ti_izn_white_olive.jpg' },
            { name: 'Фуксия', image: 'https://static.tildacdn.com/tild6337-6237-4230-a134-376532333164/ti_izn_white_pink.jpg' },
          ],
        },
        {
          name: 'Графитовый',
          image: 'https://optim.tildacdn.com/tild6164-3232-4962-a538-353530356231/-/format/webp/ti_izn_graph_graph.jpg',
          seams: [
            { name: 'Графитовый' },
            { name: 'Розовый', image: 'https://optim.tildacdn.com/tild6562-3866-4666-b730-313465373734/-/format/webp/ti_izn_graph_pink.jpg' },
            { name: 'Олива', image: 'https://static.tildacdn.com/tild3338-3530-4730-b764-306361383165/ti_izn_graph_olive.jpg' },
          ],
        },
        { name: 'Розовый', image: 'https://optim.tildacdn.com/tild3636-6134-4762-b761-353139346161/-/format/webp/ti_izn_pink.jpg' },
        { name: 'Коричневый', image: 'https://optim.tildacdn.com/tild3033-3664-4437-b930-326139663137/-/format/webp/ti_izn_brown.jpg' },
        { name: 'Серый', image: 'https://static.tildacdn.com/tild3432-3333-4762-b938-636566393937/ti_izn_gray.jpg' },
      ],
      'tee-base': [
        { name: 'Белый', image: 'https://static.tildacdn.com/tild3330-6566-4539-b631-313661313239/ti_base_white.jpg' },
        { name: 'Чёрный', image: 'https://static.tildacdn.com/tild6633-6539-4230-b461-326166363964/ti_base_black.jpg' },
      ],
      'longsleeve-choice': [
        { name: 'Белый', image: 'https://static.tildacdn.com/tild6663-6637-4135-a436-653564303366/long_izn_white.jpg' },
        { name: 'Графит', image: 'https://optim.tildacdn.com/tild6231-3664-4563-b439-643034613865/-/format/webp/long_izn_graph.jpg.webp' },
        { name: 'Розовый', image: 'https://static.tildacdn.com/tild6136-3535-4136-b465-656333666539/long_izn_pink.jpg' },
        { name: 'Коричневый', image: 'https://static.tildacdn.com/tild6631-3531-4631-a534-666563653439/korich_long.jpg' },
        { name: 'Серый', image: 'https://optim.tildacdn.com/tild6661-6530-4236-b464-343566633937/-/format/webp/long_izn_gray.jpg.webp' },
        { name: 'Чёрный', image: 'https://optim.tildacdn.com/tild6461-3138-4361-b166-376438626534/-/format/webp/long_izn_black.jpg.webp' },
      ],
      'hoodie-basic': [
        { name: 'Чёрный', image: 'https://static.tildacdn.com/tild3538-3730-4237-b534-376637623764/hoodie_black.jpg' },
        { name: 'Графит', image: 'https://static.tildacdn.com/tild6632-3465-4132-a563-613962386465/hoodie_gray.jpg' },
        { name: 'Розовый', image: 'https://static.tildacdn.com/tild3337-3364-4465-b564-346532613038/hoodie_pink.jpg' }
      ],
      'sweatshirt-iznanka': [
        { name: 'Графит', image: 'https://optim.tildacdn.com/tild6535-6239-4138-a165-356465643862/-/format/webp/sweet_graph.jpg' },
        { name: 'Розовый', image: 'https://optim.tildacdn.com/tild3732-6335-4666-b934-373131653835/-/format/webp/sweet_pink.jpg.webp' },
        { name: 'Чёрный', image: 'https://optim.tildacdn.com/tild6333-3363-4733-a230-656665663566/-/format/webp/sweet_gray.jpg.webp' },
      ],
    };
    const sizeChartImages = {
      'tee-iznanka': 'https://www.dropbox.com/scl/fo/u5cy8odbmqdt4ek1dw3rw/AAqpSQJLmWf_CYOejI3KJlA/%D1%84%D1%83%D1%82%D0%B1%D0%B0%20%D0%B8%D0%B7%D0%BD%D0%B0%D0%BD%D0%BA%D0%B0.jpg?rlkey=turq9gsh87hs9c5l8snexigq5&st=9xq6tm48&raw=1',
      'tee-base': 'https://www.dropbox.com/scl/fo/u5cy8odbmqdt4ek1dw3rw/ALENHo0L0rNQuXKb_bLjvCM/%D1%84%D1%83%D1%82%D0%B1%D0%B0%20%D0%B1%D0%B0%D0%B7%D0%B0.jpg?rlkey=turq9gsh87hs9c5l8snexigq5&st=q21vei6q&raw=1',
      'longsleeve-choice': 'https://www.dropbox.com/scl/fo/u5cy8odbmqdt4ek1dw3rw/AJ5WncMQJ8tAuePB_6I2U-g/%D0%BB%D0%BE%D0%BD%D0%B3%20%D0%B8%D0%B7%D0%BD%D0%B0%D0%BD%D0%BA%D0%B0.jpg?rlkey=turq9gsh87hs9c5l8snexigq5&st=m6uyc3ju&raw=1',
      'hoodie-basic': 'https://www.dropbox.com/scl/fo/u5cy8odbmqdt4ek1dw3rw/APzgKmVnp-OtumJLY8mgkFA/%D1%85%D1%83%D0%B4%D0%B8%20%D0%B1%D0%B0%D0%B7%D0%B0.jpg?rlkey=turq9gsh87hs9c5l8snexigq5&st=ctq668dw&raw=1',
      'sweatshirt-iznanka': 'https://www.dropbox.com/scl/fo/u5cy8odbmqdt4ek1dw3rw/ADdaRgRzIUun8_brJTIi3t8/%D1%81%D0%B2%D0%B8%D1%82%20%D0%B8%D0%B7%D0%BD%D0%B0%D0%BD%D0%BA%D0%B0.jpg?rlkey=turq9gsh87hs9c5l8snexigq5&st=foh9j9f8&raw=1',
    };
    const seamColors = ['Белый', 'Графитовый'];
    const colors = [
      { name: 'Белый', value: '#F4F4F4', enabled: true },
      { name: 'Чёрный', value: '#111111', enabled: false },
      { name: 'Серый', value: '#bdbdbd', enabled: false },
      { name: 'Розовый', value: '#E99BB0', enabled: false },
      { name: 'Графитовый', value: '#474448', enabled: false },
...
// ===== constructor-checkout-mvp-tilda-js-1.html =====
    ...
    const extras = [
      { id: 'hand-drawn', name: 'Рисованные элементы на фотографии', description: 'К любому шаблону с фотографией вы можете добавить рисованные элементы. Макет будет выслан после работы дизайнера.', price: 300, icon: 'https://optim.tildacdn.com/tild3861-6631-4133-b232-363732663865/-/resize/200x/-/format/webp/photo.jpg' },
      { id: 'polaroid-photo', name: 'Полароид', description: 'Ваши фотографии будут напечатаны на оригинальных картриджах Fujifilm', price: 390, icon: 'https://optim.tildacdn.com/tild3538-3033-4136-b039-623661366362/-/format/webp/photo.jpg' },
      { id: 'cassette', name: 'Аудио открытка «кассета»', description: 'На кассету можно записать своё 60-секундное послание, которое получатель сможет прослушать.', price: 590, icon: 'https://optim.tildacdn.com/tild6266-3536-4232-a338-306561316665/-/format/webp/photo.jpg' },
    ];
    // количество допов
    const extrasCount = {};
    ...
    const anjhayRuntimeFontFamily = 'Anjhay';
    const anjhayRuntimeFontUrl = 'https://www.dropbox.com/scl/fo/04y8ovljbyi645ppvs0xe/AMy9HR3EGrpN60qslRe5sPM/Anjhay_%D1%88%D1%80%D0%B8%D1%84%D1%82%203.otf?rlkey=pnn5zy5e82rhap6fbta5ks53v&st=flvww00w&dl=0';
    const zhiznRuntimeFontFamily = 'Zhizn Regular';
    const zhiznRuntimeFontUrl = 'https://www.dropbox.com/scl/fo/04y8ovljbyi645ppvs0xe/AHp531W47X9Xcdmp7XdN6dA/zhizn_%D1%88%D1%80%D0%B8%D1%84%D1%82%204.otf?rlkey=pnn5zy5e82rhap6fbta5ks53v&st=z1hq49e5&dl=0';
    const externalFontOptionSources = {
      ...
      'emb-center-print-font-3': {
        family: anjhayRuntimeFontFamily,
        url: anjhayRuntimeFontUrl,
        format: 'opentype',
      },
      'emb-center-print-font-4': {
        family: zhiznRuntimeFontFamily,
        url: zhiznRuntimeFontUrl,
        format: 'opentype',
      },
    ...
    let baseImageSpinnerTimer = null;
    ...
    const KONVA_TEMPLATE_CONFIGS = {
      ...
      'simple-photo': {
        ...
        photoDefaults: { exportX: 453, exportY: 483, scale: 0.853 },
        printSizeOverrides: {
          s: { photoExportX: 453 },
      ...
      'polaroid-photo': {
        ...
        photoPosition: 'center',
        photoDefaults: { exportX: 457.1, exportY: 486.9, scaleX: 0.782, scaleY: 0.802 },
        frameDefaults: { exportX: 453, exportY: 483, scale: 0.947 },
        printSizeOverrides: {
          s: { photoExportX: 457.1, photoExportY: 486.9, photoScaleX: 0.782, photoScaleY: 0.802, frameExportX: 453, frameExportY: 483, frameScale: 0.947 },
          l: { photoExportX: 448, photoExportY: 487, photoScaleX: 0.670, photoScaleY: 0.697, frameExportX: 443.6, frameExportY: 483, frameScale: 0.822 },
        },
        layerOrder: ['photo', 'polaroidFrame'],
      },
      'photo-top-text': {
        hasText: true, hasPhoto: true, hasFrame: false,
        textPosition: 'above-photo', photoPosition: 'below-text',
        photoDefaults: { exportX: 453, exportY: 503, scale: 0.853 },
      ...
      'polaroid-top-text': {
        ...
        frameDefaults: { exportX: 453, exportY: 503, scale: 0.947 },
        textDefaults: { exportX: 500, exportCenterY: 490, anchor: 'middle' },
        textScaleMultiplier: 1.381,
        printSizeOverrides: {
          l: { photoExportX: 448, photoExportY: 488, photoScaleX: 0.670, photoScaleY: 0.697, frameExportX: 443.6, frameExportY: 503, frameScale: 0.822 },
          m: { exportCenterY: 493, textScaleMultiplier: 1.503 },
          's:s': { photoExportY: 499, frameExportY: 496 },
          'm:s': { photoExportY: 506, frameExportY: 504 },
          's:l': { photoExportX: 448, photoExportY: 501, photoScaleX: 0.670, photoScaleY: 0.697, frameExportX: 443.6, frameExportY: 496, frameScale: 0.822 },
          'm:l': { photoExportX: 448, photoExportY: 509, photoScaleX: 0.670, photoScaleY: 0.697, frameExportX: 443.6, frameExportY: 503, frameScale: 0.822 },
        },
        textGap: { preview: 16, export: 20 }, textDescentRatio: 0.32,
        layerOrder: ['photo', 'polaroidFrame', 'text'],
      },
      'photo-bottom-text': {
        hasText: true, hasPhoto: true, hasFrame: false,
        textPosition: 'below-photo', photoPosition: 'center',
        photoDefaults: { exportX: 453, exportY: 483, scale: 0.853 },
        textDefaults: { exportX: 500, exportCenterY: 607.5, anchor: 'middle' },
        textScaleMultiplier: 1.381,
        printSizeOverrides: {
          l: { photoExportX: 436, photoExportY: 483, photoScale: 0.822 },
          s: { exportCenterY: 493, textScaleMultiplier: 1.503 },
          's:s': { photoExportY: 483, exportCenterY: 611.5 },
          'm:s': { photoExportY: 483, exportCenterY: 611.5 },
      ...
      'photo-diagonal-text': {
        ...
        photoDefaults: { exportX: 453, exportY: 483, scale: 0.853 },
        textDefaults: { exportX: 540, exportCenterY: 601, anchor: 'middle' },
        textScaleMultiplier: 1.381,
        printSizeOverrides: {
          l: { photoExportX: 453, photoExportY: 483, photoScale: 0.853, diagonalExportX: 559.6, diagonalExportY: 644.2 },
          m: { exportCenterY: 493, textScaleMultiplier: 1.503 },
          s: { diagonalExportX: 540, diagonalExportY: 601 },
          's:s': { photoExportY: 483, diagonalExportX: 540, diagonalExportY: 601 },
          'm:s': { photoExportY: 483, diagonalExportX: 540, diagonalExportY: 601 },
          's:l': { photoExportX: 436, photoExportY: 483, photoScale: 0.822, diagonalExportX: 559.6, diagonalExportY: 644.2 },
          'm:l': { photoExportX: 436, photoExportY: 483, photoScale: 0.822, diagonalExportX: 559.6, diagonalExportY: 644.2 },
        },
        diagonalText: { rotation: -24, xRatio: 0.87, previewTopRatio: 0.81, exportBaselineRatio: 0.9, lineHeightScale: 0.85 },
        diagonalFontOverrides: {
          'emb-center-print-font-2': {
            rotation: -24,
            's:s': { diagonalExportX: 539.8, diagonalExportY: 601.1 },
            'm:s': { diagonalExportX: 537.7, diagonalExportY: 600.1 },
            's:l': { diagonalExportX: 559.4, diagonalExportY: 644.3 },
            'm:l': { diagonalExportX: 559.4, diagonalExportY: 645.3 },
          },
          'emb-center-print-font-3': {
            rotation: -24,
            's:s': { diagonalExportX: 541.9, diagonalExportY: 606.1 },
            'm:s': { diagonalExportX: 540.9, diagonalExportY: 607.1 },
            's:l': { diagonalExportX: 562.9, diagonalExportY: 651.3 },
            'm:l': { diagonalExportX: 562.3, diagonalExportY: 652.5 },
          },
        },
        layerOrder: ['photo', 'text'],
      },
      'polaroid-diagonal-text': {
        ...
        textScaleMultiplier: 1.381,
        printSizeOverrides: {
          s: { photoExportX: 457.1, photoExportY: 486.9, photoScaleX: 0.782, photoScaleY: 0.802, frameExportX: 453, frameExportY: 483, frameScale: 0.947, diagonalExportX: 538.7, diagonalExportY: 615 },
          l: { photoExportX: 448, photoExportY: 488, photoScaleX: 0.670, photoScaleY: 0.697, frameExportX: 443.6, frameExportY: 483, frameScale: 0.822, diagonalExportX: 548.5, diagonalExportY: 635.6 },
          m: { exportCenterY: 493, textScaleMultiplier: 1.503 },
          's:l': { photoExportX: 448, photoExportY: 488, photoScaleX: 0.670, photoScaleY: 0.697, frameExportX: 443.6, frameExportY: 483, frameScale: 0.822, diagonalExportX: 548.5, diagonalExportY: 635.6 },
          'm:s': { photoExportX: 457.1, photoExportY: 486.9, photoScaleX: 0.782, photoScaleY: 0.802, frameExportX: 453, frameExportY: 483, frameScale: 0.947, diagonalExportX: 531.9, diagonalExportY: 616.5 },
          'm:l': { photoExportX: 448, photoExportY: 488, photoScaleX: 0.670, photoScaleY: 0.697, frameExportX: 443.6, frameExportY: 483, frameScale: 0.822, diagonalExportX: 541.3, diagonalExportY: 644.2 },
        },
        diagonalText: { rotation: -24, xRatio: 0.87, previewTopRatio: 0.81, exportBaselineRatio: 0.9, lineHeightScale: 0.85 },
        diagonalFontOverrides: {
          'emb-center-print-font-3': {
            rotation: -24,
            'm:s': { diagonalExportX: 551.2, diagonalExportY: 603 },
            'm:l': { diagonalExportX: 556.1, diagonalExportY: 644.2 },
          },
        },
        layerOrder: ['photo', 'polaroidFrame', 'text'],
      },
      'polaroid-inside-text': {
        ...
        photoDefaults: { exportX: 457.1, exportY: 486.9, scaleX: 0.782, scaleY: 0.802 },
        frameDefaults: { exportX: 453, exportY: 483, scale: 0.947 },
        textDefaults: { exportX: 535.2, exportCenterY: 580.3, anchor: 'end' },
        textScaleMultiplier: 0.24,
        printSizeOverrides: {
          s: { photoExportX: 457.1, photoExportY: 486.9, photoScaleX: 0.782, photoScaleY: 0.802, frameExportX: 453, frameExportY: 483, frameScale: 0.947, exportX: 541.2, exportCenterY: 594.2, textScaleMultiplier: 0.425 },
          's:l': { photoExportX: 448, photoExportY: 488, photoScaleX: 0.670, photoScaleY: 0.697, frameExportX: 443.6, frameExportY: 483, frameScale: 0.822, exportX: 553.1, exportCenterY: 621.6, textScaleMultiplier: 0.518 },
        },
        layerOrder: ['photo', 'polaroidFrame', 'text'],
      },
    };
    ...
    function konvaResolveScene(params) {
      const {
        templateKind, fontId = '', textLines = [], textColor = '#111111',
      ...
      if (hideDesign || (!config && !isCustom)) {
        return scene;
      }
      if (isCustom) {
        const cpSrc = params.customPreviewSrc || null;
        if (cpSrc) {
          const tplDefaults = params.customPreviewDefaults || null;
          const spConfig = KONVA_TEMPLATE_CONFIGS['simple-photo'];
          const spDefaults = spConfig?.photoDefaults || {};
          const spScale = spDefaults.scale || 0.8;
          const imgW = tplDefaults?.width ?? (photoExportWidth * spScale);
          const imgH = tplDefaults?.height ?? (photoExportHeight * spScale);
          const imgX = tplDefaults?.exportX ?? spDefaults.exportX ?? (KONVA_EXPORT_W - imgW) / 2;
          const imgY = tplDefaults?.exportY ?? spDefaults.exportY ?? 483;
          scene.elements.push({ type: 'custom-preview', name: 'customPreview', x: imgX, y: imgY, width: imgW, height: imgH, src: cpSrc });
        }
        return scene;
      }
      ...
      if (config.hasText && textLines.length > 0) {
        ...
        if (config.textPosition === 'diagonal-photo' && config.diagonalText) {
          ...
          const fontOv = config.diagonalFontOverrides?.[fontId];
          const fontComboOv = fontOv?.[comboKey] || {};
          const overrideX = fontComboOv.diagonalExportX ?? comboOverride.diagonalExportX ?? psOverride.diagonalExportX ?? dt.exportX ?? null;
          const overrideY = fontComboOv.diagonalExportY ?? comboOverride.diagonalExportY ?? psOverride.diagonalExportY ?? dt.exportY ?? null;
          const textX = overrideX != null ? overrideX : ref.x + ref.width * dt.xRatio;
          const textY = overrideY != null ? overrideY : ref.y + ref.height * dt.exportBaselineRatio;
          const diagLHScale = fontOv?.lineHeightScale ?? dt.lineHeightScale;
          const diagLH = diagLHScale != null ? lineHeight * diagLHScale : lineHeight;
          const diagRotation = fontOv?.rotation ?? dt.rotation;
          textLayout = { x: textX, y: textY, fontSize, lineHeight: diagLH, anchor: 'middle', rotation: diagRotation, clipRect: null };
    ...
    function konvaUpdateDebugPanel(node) {
      ...
      const isCustomPreviewNode = name === 'customPreview';
      ...
      if (isCustomPreviewNode) {
        const customTpl = typeof getSelectedCustomTemplate === 'function' ? getSelectedCustomTemplate() : null;
        const cpd = customTpl?.photoDefaults || {};
        const cs = cpd.scale ?? 1;
        configMulX = cpd.scaleX ?? cs;
        configMulY = cpd.scaleY ?? cs;
      } else if (isFrameNode) {
    ...
    async function konvaBuildSceneNodes(layer, scene, devMode) {
      ...
      for (const el of scene.elements) {
        ...
        switch (el.type) {
          ...
          case 'custom-preview': await konvaBuildCustomPreview(layer, el, isDraggable); break;
        }
      }
    }
    ...
    async function konvaBuildCustomPreview(layer, el, draggable) {
      let img;
      try { img = await konvaLoadImage(el.src); } catch (_) { return; }
      if (!img) return;
      const nw = img.naturalWidth || img.width, nh = img.naturalHeight || img.height;
      ...
      const dw = nw * fitS, dh = nh * fitS;
      const node = new Konva.Image({
        name: el.name || 'customPreview', image: img,
    ...
    let konvaFlatPreviewRenderToken = 0;
    // sceneKey -> data URL. Раз отрендерили — больше не рендерим. Прогревается
    // на старте (см. pregenerateFlatPreviewsForAllVariants) для всех
    // комбинаций изделие × цвет × шов, поэтому переключения мгновенные.
    const konvaFlatPreviewCache = new Map();
    ...
    function konvaEnsurePreviewStage() {
      ...
      // pixelRatio = max(3, DPR) — рисуем canvas в 3x разрешении дисплея.
      // При hover-zoom (3x CSS scale) net upscale ~1x → нет пикселизации.
      // Без этого canvas выглядел «шакальным» при зуме.
      Konva.pixelRatio = Math.max(3, window.devicePixelRatio || 1);
    ...
    async function konvaRenderPreview(scene, sceneKey) {
      ...
      // false → элементы не draggable. KONVA_DEV_MODE = true исторически
       // делал их draggable для отладки, но в live-canvas режиме на step 4
       // мы не хотим возможности юзера двигать элементы случайно — они
       // редактируются через панель справа (текст, фото). Drag фотки уже
       // обрабатывается через pointer-handler на garmentWrap отдельно.
      await konvaBuildSceneNodes(konvaPreviewLayer, scene, false);
    ...
    async function konvaExportToDataURL(scene, options = {}) {
      ...
      try {
        ...
        layer.batchDraw();
        // pixelRatio: 2 — PNG экспорт получает в 2 раза больше пикселей в
        // каждом направлении (1000×1250 logical → 2000×2500 raster). Это
        // нужно, чтобы при CSS-zoom (3x на десктоп hover) превью не было
        // пиксельным. pixelRatio: 1 давал «шакальное» качество при зуме.
        const dataURL = stage.toDataURL({ mimeType, quality, pixelRatio: 2 });
        stage.destroy();
        return dataURL;
      } finally {
    ...
    function loadKonvaScript() {
      ...
      return new Promise((resolve, reject) => {
        ...
        script.onload = () => { konvaReady = true; resolve(); };
    ...
    function buildKonvaSceneParams() {
      ...
      return {
        ...
        exportFontScale: efs,
        fontWeight: getFontWeight(state.font, selectedTemplate) ?? textRules.fontWeight ?? DEFAULT_PREVIEW_TEXT_RULES.fontWeight,
        ...
        customPreviewSrc: state.selectedCustom ? (getSelectedCustomTemplate()?.previewImage || null) : null,
        customPreviewDefaults: state.selectedCustom ? (getSelectedCustomTemplate()?.photoDefaults || null) : null,
    ...
    // =====================================================================
    // PREVIEW ZOOM (step 4 — desktop hover zoom only)
    // =====================================================================
    let previewZoomController = null;

    function destroyPreviewZoom() {
      if (previewZoomController) {
        previewZoomController.abort();
        previewZoomController = null;
      }
      // На step 4 зум persist'ится между перерисовками (через CSS-переменные).
      // На других шагах ОБНУЛЯЕМ полностью — иначе зум со step 4 «остаётся»
      // когда юзер переходит на другие шаги.
      const gw = document.getElementById('garmentWrap');
      if (gw) {
        gw.classList.remove('preview-touch-zoom');
        if (state.step !== 4) {
          gw.style.removeProperty('--pinch-zoom');
          gw.style.removeProperty('--pinch-tx');
          gw.style.removeProperty('--pinch-ty');
          gw.classList.remove('zoom-active');
          __persistedZoomState = { zoom: 1, tx: 0, ty: 0 };
        }
      }
    }

    // Persist zoom state across renders. setupPreviewZoom destroy'ится при
    // каждом renderVisualizer'е, но user'ский зум должен сохраняться.
    let __persistedZoomState = { zoom: 1, tx: 0, ty: 0 };
    ...
    function setupPreviewZoom() {
      ...
      previewZoomController = new AbortController();
      const signal = previewZoomController.signal;

      // Восстанавливаем зум из persisted state (а не сбрасываем на 1).
      let currentZoom = __persistedZoomState.zoom || 1;
      let currentTx = __persistedZoomState.tx || 0;
      let currentTy = __persistedZoomState.ty || 0;
      // Применяем восстановленный transform через CSS-переменные на дочерних
      // элементах wrap'а (clip-zoom: контейнер сам не увеличивается, content
      // внутри scales и обрезается через overflow:hidden на wrap).
      const writeZoomCss = () => {
        const wasZoomActive = garmentWrap.classList.contains('zoom-active');
        garmentWrap.style.setProperty('--pinch-zoom', String(currentZoom));
        garmentWrap.style.setProperty('--pinch-tx', `${currentTx}px`);
        garmentWrap.style.setProperty('--pinch-ty', `${currentTy}px`);
        const willBeZoomActive = currentZoom > 1.001;
        garmentWrap.classList.toggle('zoom-active', willBeZoomActive);
        // Если .zoom-active toggled — wrap aspect-ratio изменился (на десктопе
        // 4:5 ↔ 1.2:1). Konva canvas рендерится с фиксированным внутренним
        // размером (1000×1250) и при изменении wrap dimensions нужен resize,
        // иначе canvas остаётся искажённым (растянутым) при возврате к 4:5.
        if (wasZoomActive !== willBeZoomActive) {
          requestAnimationFrame(() => {
            try {
              if (typeof konvaResizePreview === 'function') konvaResizePreview();
            } catch (_) {}
          });
        }
      };
      writeZoomCss();
      const persist = () => {
        __persistedZoomState = { zoom: currentZoom, tx: currentTx, ty: currentTy };
      };

      const applyTransform = (animate) => {
        // Clip-zoom: применяем transform через CSS-переменные на детей wrap'а,
        // а не на сам wrap. Контейнер не меняет внешний размер, контент
        // внутри scales и обрезается через overflow:hidden + .zoom-active.
        // animate флаг не используем (transition только мешает live pinch).
        currentZoom = Math.max(1, Math.min(3, currentZoom || 1));
        writeZoomCss();
        persist();
      };
      ...
      const resetTransform = (animate) => {
        ...
        currentTx = 0;
        currentTy = 0;
        garmentWrap.classList.remove('preview-touch-zoom');
        // writeZoomCss уберёт .zoom-active, сбросит CSS vars и вызовет
        // konvaResizePreview (нужно при возврате aspect-ratio с 1.2:1 к 4:5,
        // иначе canvas остаётся искажённым).
        writeZoomCss();
        __persistedZoomState = { zoom: 1, tx: 0, ty: 0 };
      };

      const clampTranslation = () => {
        const parent = garmentWrap.parentElement;
        if (!parent) return;
        const rect = parent.getBoundingClientRect();
        const maxTx = rect.width * (currentZoom - 1) / 2;
        const maxTy = rect.height * (currentZoom - 1) / 2;
        currentTx = clamp(currentTx, -maxTx, maxTx);
        currentTy = clamp(currentTy, -maxTy, maxTy);
      };

      const isDesktop = window.matchMedia('(hover: hover) and (pointer: fine) and (min-width: 769px)').matches;

      if (isDesktop) {
        // Desktop step 4: фиксированный crop-zoom. Никаких hover-эффектов
        // и интерактивных лупп — превью просто всегда отображается с
        // постоянным увеличением вокруг центра. Всё содержимое wrap'а
        // (футболка + впечатанные текст/фото/рамки) равномерно
        // масштабируется одним и тем же CSS scale, поэтому относительные
        // позиции элементов не меняются — центральная часть видна крупнее,
        // края обрезаются overflow:hidden (правило .zoom-active в CSS).
        //
        // Качество: источник — live Konva canvas с pixelRatio=3
        // (Konva.pixelRatio в konvaEnsurePreviewStage), то есть внутренний
        // raster в 3 раза плотнее CSS-размеров. CSS-апскейл 1.5×
        // (DESKTOP_FIXED_ZOOM) × 1.18 (--garment-content-scale) = ~1.77,
        // что всё ещё в 1.7 раза меньше плотности canvas → пикселизация
        // не возникает.
        const DESKTOP_FIXED_ZOOM = 1.5;
        currentZoom = DESKTOP_FIXED_ZOOM;
        currentTx = 0;
        currentTy = 0;
        // writeZoomCss добавит .zoom-active (overflow:hidden + transform
        // через CSS-переменные) и вызовет konvaResizePreview, чтобы canvas
        // пересчитал свои внутренние размеры под актуальный wrap.
        writeZoomCss();
        persist();
      } else {
        // Mobile: pinch-to-zoom + pan
        garmentWrap.style.touchAction = 'none';
        signal.addEventListener('abort', () => {
          garmentWrap.style.touchAction = '';
        });
        const previewZoomHint = document.getElementById('previewZoomHint');
        if (previewZoomHint) {
          previewZoomHint.classList.remove('is-hidden');
          const hideHint = () => {
            previewZoomHint.classList.add('is-hidden');
          };
          garmentWrap.addEventListener('pointerdown', hideHint, { signal, once: true });
        }
        const MAX_ZOOM = 3;
        ...
        const getPoints = () => Array.from(activePointers.values()).slice(0, 2);
        ...
        garmentWrap.addEventListener('pointerdown', (e) => {
          ...
          if (activePointers.size === 2) {
            ...
            panState = null;
          } else if (activePointers.size === 1 && currentZoom > 1) {
            panState = {
              startX: e.clientX,
              startY: e.clientY,
              startTx: currentTx,
        ...
        garmentWrap.addEventListener('pointermove', (e) => {
          ...
          if (pinchState && activePointers.size >= 2) {
            ...
            const zoomFactor = newDist / pinchState.startDist;
            currentZoom = clamp(pinchState.startZoom * zoomFactor, 1, MAX_ZOOM);
            currentTx = pinchState.startTx + (newMidX - pinchState.startMidX);
            currentTy = pinchState.startTy + (newMidY - pinchState.startMidY);
            clampTranslation();
            applyTransform(false);
            if (currentZoom > 1) {
              garmentWrap.classList.add('preview-touch-zoom');
            }
            return;
          }

          if (panState && currentZoom > 1) {
            currentTx = panState.startTx + (e.clientX - panState.startX);
            currentTy = panState.startTy + (e.clientY - panState.startY);
            clampTranslation();
            applyTransform(false);
            return;
          }
        ...
        garmentWrap.addEventListener('pointerup', stopGesture, { signal });
        garmentWrap.addEventListener('pointercancel', stopGesture, { signal });

        // Double-tap to reset
        let lastTapTime = 0;
        garmentWrap.addEventListener('click', () => {
          const now = Date.now();
          if (now - lastTapTime < 320 && currentZoom > 1) {
            resetTransform(true);
          }
          lastTapTime = now;
        }, { signal });
    ...
    function getTemplateCardMeta(item, mode = 'template') {
      ...
      return item.uploads ? `${item.uploads} фото` : 'Печать / вышивка';
    }
    ...
    function renderCustomContactCard() {
      return `
        <div class="template-contact-card">
          <div class="template-contact-card-copy">
            <div class="template-contact-card-title">Не нашли подходящий дизайн?</div>
            <div class="template-contact-card-text">Свяжитесь с нашим менеджером — поможем подобрать или разработать индивидуальный вариант.</div>
          </div>
          <a class="template-contact-card-btn" href="https://t.me/patalahas" target="_blank" rel="noopener noreferrer">Связаться</a>
        </div>
      `;
    }
    function renderTemplateChoiceCard(item, options = {}) {
      ...
      const priceLabel = `от ${formatPrice(item.price)}`;
      const priceMark = '';
      return `
    ...
    function shouldResetLegacyDefaultTemplate(snapshotState, snapshotExtras = {}, snapshotUploads = [], snapshotVersion = 0) {
      ...
      if (snapshotState.fastProduction || snapshotState.superFastProduction || snapshotState.isEmbroideryText) return false;
    ...
    // Защита zoom от незапланированных сбросов: переход >1.01 → ≤1.01
    // блокируется кроме случаев явных user-actions (file upload/remove).
    let __cropZoomValue = state.cropZoom;
    let __cropZoomAllowReset = false;
    Object.defineProperty(state, 'cropZoom', {
      configurable: true,
      enumerable: true,
      get() { return __cropZoomValue; },
      set(next) {
        const numericNext = Number(next) || 1;
        if (
          !__cropZoomAllowReset
          && __cropZoomValue > 1.01
          && numericNext <= 1.01
        ) {
          return; // блокируем сброс
        }
        __cropZoomValue = numericNext;
      },
    });

    // === Disable scroll-on-focus for inputs ===
    // На мобиле тап по text input вызывает scroll-into-view браузером.
    // Это раздражает (страница «прыгает»). Восстанавливаем scroll сразу.
    if (typeof window !== 'undefined' && typeof document !== 'undefined') {
      document.addEventListener('focusin', (e) => {
        ...
        // Несколько попыток восстановления — браузер скроллит async.
        requestAnimationFrame(() => window.scrollTo(savedX, savedY));
        setTimeout(() => window.scrollTo(savedX, savedY), 0);
        setTimeout(() => window.scrollTo(savedX, savedY), 50);
      }, true);
    }
    // Хелпер для случаев, когда zoom РЕАЛЬНО надо сбросить (file change).
    function __resetCropZoomAllowed(value = 1) {
      __cropZoomAllowReset = true;
      try { state.cropZoom = value; } finally { __cropZoomAllowReset = false; }
    }
    if (typeof window !== 'undefined') {
      window.__resetCropZoomAllowed = __resetCropZoomAllowed;
    }

    // Раньше здесь принудительно ставили meta viewport user-scalable=no.
    // Не помогало — Tilda перезаписывает meta тег обратно. Убрано в пользу
    // решения через font-size: 16px на inputs (предотвращает Chrome
    // auto-zoom-on-focus → пользовательский pinch-zoom сохраняется при
    // тапе по тексту).
    // Раньше здесь была агрессивная блокировка multi-touch / gesture events.
    // Это не помогло (Tilda всё равно отдавала pinch браузеру) И мешало
    // нормальному взаимодействию. Удалено в пользу решения через font-size:
    // 16px на inputs (предотвращает Chrome auto-zoom-on-focus, который и
    // вызывал «zoom reset» при тапе по тексту).

    function getProductionMode(snapshotState = state) {
      if (snapshotState?.superFastProduction) return 'superfast';
      if (snapshotState?.fastProduction) return 'fast';
      return 'standard';
    }
    function applyProductionMode(mode = 'standard', targetState = state) {
      targetState.fastProduction = mode === 'fast';
      targetState.superFastProduction = mode === 'superfast';
      return getProductionMode(targetState);
    }
    function getProductionModeStatusLabel(snapshotState = state) {
      const productionMode = getProductionMode(snapshotState);
      if (productionMode === 'superfast') return '✓ За 24 часа';
      if (productionMode === 'fast') return '✓ За 2-3 дня';
      return 'Стандартное';
    }
    function getProductionModeSummaryLabel(snapshotState = state) {
      const productionMode = getProductionMode(snapshotState);
      if (productionMode === 'superfast') return 'Изготовление за 24 часа';
      if (productionMode === 'fast') return 'Ускоренное изготовление';
      return 'Стандартное изготовление';
    }
    ...
    const syncTextColorToGarment = () => {
      const darkColors = ['Чёрный', 'Графит', 'Графитовый'];
      if (darkColors.includes(state.color)) state.textColor = 'rgb(255, 255, 255)';
      else state.textColor = '#000000';
    };
    ...
    function normalizeFontOption(fontOption, index = 0) {
      ...
      return {
        ...
        lowercaseOnly: Boolean(fontOption.lowercaseOnly),
        ...
        lineHeightScale: fontOption.lineHeightScale != null ? Number(fontOption.lineHeightScale) : undefined,
        fontWeight: fontOption.fontWeight != null ? Number(fontOption.fontWeight) : undefined,
        fontSizeAdjust: fontOption.fontSizeAdjust != null ? Number(fontOption.fontSizeAdjust) : undefined,
        lowercasePreviewScale: fontOption.lowercasePreviewScale != null ? Number(fontOption.lowercasePreviewScale) : undefined,
        lowercaseExportScale: fontOption.lowercaseExportScale != null ? Number(fontOption.lowercaseExportScale) : undefined,
        lowercaseLineHeightScale: fontOption.lowercaseLineHeightScale != null ? Number(fontOption.lowercaseLineHeightScale) : undefined,
        lowercaseFontWeight: fontOption.lowercaseFontWeight != null ? Number(fontOption.lowercaseFontWeight) : undefined,
      };
    }
    ...
    const getSelectedDesignPrice = () => {
      const custom = getSelectedCustomTemplate();
      if (custom) return custom.price || 0;
      const template = getSelectedTemplate();
      if (!template) return 0;
      if (template.prices) {
        const photoSize = getSelectedPhotoPrintSize(undefined, template);
        const sizeId = photoSize?.id || state.printSize || 's';
        return template.prices[sizeId] || template.price || 0;
      }
    ...
    function getPreviewFontScale(fontId = state.font, template = getSelectedTemplate()) {
      const opt = getSelectedFontOption(fontId, template);
      if (!opt) return 1;
      return (textHasLowercase() && opt.lowercasePreviewScale != null) ? opt.lowercasePreviewScale : (opt.previewScale || 1);
    }
    function getExportFontScale(fontId = state.font, template = getSelectedTemplate()) {
      const opt = getSelectedFontOption(fontId, template);
      if (!opt) return 1;
      return (textHasLowercase() && opt.lowercaseExportScale != null) ? opt.lowercaseExportScale : (opt.exportScale || 1);
    }
    function getLineHeightScale(fontId = state.font, template = getSelectedTemplate()) {
      const opt = getSelectedFontOption(fontId, template);
      if (!opt) return 1;
      return (textHasLowercase() && opt.lowercaseLineHeightScale != null) ? opt.lowercaseLineHeightScale : (opt.lineHeightScale || 1);
    }
    function getFontWeight(fontId = state.font, template = getSelectedTemplate()) {
      const opt = getSelectedFontOption(fontId, template);
      if (!opt) return null;
      if (textHasLowercase() && opt.lowercaseFontWeight != null) return opt.lowercaseFontWeight;
      return opt.fontWeight ?? null;
    }
    ...
    function isFontLowercaseOnly(fontId = state.font, template = getSelectedTemplate()) {
      return Boolean(getSelectedFontOption(fontId, template)?.lowercaseOnly);
    }
    ...
    function getPhotoDiagonalTextLayout(
      ...
      const computedExportX = Number(photoLayout.exportX || 0) + (Number(photoLayout.exportWidth || 0) * PHOTO_DIAGONAL_TEXT_X_RATIO);
      const computedBaselineY = Number(photoLayout.exportY || 0) + (Number(photoLayout.exportHeight || 0) * PHOTO_DIAGONAL_TEXT_EXPORT_BASELINE_RATIO);
      const konvaConfig = KONVA_TEMPLATE_CONFIGS[template?.kind];
      const comboKey = (printSize?.id && selectedPhotoPrintSize?.id) ? `${printSize.id}:${selectedPhotoPrintSize.id}` : '';
      const psOverride = (konvaConfig?.printSizeOverrides && (konvaConfig.printSizeOverrides[comboKey] || konvaConfig.printSizeOverrides[printSize?.id])) || {};
      const fontOv = konvaConfig?.diagonalFontOverrides?.[state.font];
      const fontComboOv = fontOv?.[comboKey] || {};
      const diagonalXOverride = fontComboOv.diagonalExportX ?? psOverride.diagonalExportX ?? konvaConfig?.diagonalText?.exportX ?? null;
      const diagonalYOverride = fontComboOv.diagonalExportY ?? psOverride.diagonalExportY ?? konvaConfig?.diagonalText?.exportY ?? null;
      const exportX = diagonalXOverride != null ? diagonalXOverride : computedExportX;
      const exportY = diagonalYOverride != null ? diagonalYOverride : computedBaselineY;
      const previewTopSource = diagonalYOverride != null
        ? diagonalYOverride - (Number(photoLayout.exportHeight || 0) * (PHOTO_DIAGONAL_TEXT_EXPORT_BASELINE_RATIO - PHOTO_DIAGONAL_TEXT_PREVIEW_TOP_RATIO))
        : Number(photoLayout.exportY || 0) + (Number(photoLayout.exportHeight || 0) * PHOTO_DIAGONAL_TEXT_PREVIEW_TOP_RATIO);
      const diagRotation = fontOv?.rotation ?? PHOTO_DIAGONAL_TEXT_ROTATION;
      return {
    ...
    function getGarmentPreviewFocusConfig(template = getSelectedTemplate()) {
      // Preview zoom (desktop only, mobile zoom disabled)
      return null;
    ...
    function getPreviewTextBlockStyleConfig(template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template), lineCount = 1) {
      ...
      return {
        ...
        lineHeight: String((rules.lineHeight || DEFAULT_PREVIEW_TEXT_RULES.lineHeight) * getLineHeightScale(state.font, template) * (isPhotoDiagonalTextTemplate(template) && KONVA_TEMPLATE_CONFIGS[template?.kind]?.diagonalText?.lineHeightScale != null ? KONVA_TEMPLATE_CONFIGS[template?.kind].diagonalText.lineHeightScale : 1)),
        fontWeight: String(getFontWeight(state.font, template) ?? rules.fontWeight ?? DEFAULT_PREVIEW_TEXT_RULES.fontWeight),
        ...
        fontSizeAdjust: getSelectedFontOption(state.font, template)?.fontSizeAdjust ?? null,
      };
    }
    function getPreviewTextRenderMetrics(template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template), lineCount = 1, options = {}) {
      ...
      if (config) {
        ...
        if (isPhotoDiagonalTextTemplate(template)) {
          const diagonalLayout = getPhotoDiagonalTextLayout(template, printSize, lineCount);
          if (diagonalLayout) {
            const diagKonva = KONVA_TEMPLATE_CONFIGS[template?.kind];
            const diagFontOv = diagKonva?.diagonalFontOverrides?.[state.font];
            const diagLHScale = diagFontOv?.lineHeightScale ?? diagKonva?.diagonalText?.lineHeightScale;
            return {
              x: diagonalLayout.exportX,
              anchor: 'middle',
              fontSize,
              lineHeight: diagLHScale != null ? lineHeight * diagLHScale : lineHeight,
    ...
    const getProductionTotal = () => {
      const productionMode = getProductionMode();
      return productionMode === 'superfast' ? 800 : productionMode === 'fast' ? 500 : 0;
    };
    ...
    const getEmbroideryTotal = () => {
      ...
      return 350 + Math.max(0, getEmbroiderySymbolsCount() - 5) * 15;
    };
    ...
    const resolvedImageDataUrls = new Map();
    ...
    // Собирает номера позиций («Кастомное изделие N»), которые сейчас РЕАЛЬНО есть
    // в корзине. По ним мы понимаем, какой товар удалили, и чистим его файлы/описание.
    function collectActiveCartItemNumbers(products = window.tcart?.products, cartWindow = getTildaCartWindow()) {
      const numbers = new Set();
      const consider = (text) => {
        String(text || '').replace(/Кастомное изделие\s+(\d+)/gi, (whole, num) => {
      ...
      const activeProducts = Array.isArray(products)
        ? products.filter((item) => item && item.deleted !== 'yes')
        : [];
      activeProducts.forEach((item) => consider(item?.name));
      let domCount = 0;
      if (cartWindow instanceof Element) {
        const productNodes = Array.from(cartWindow.querySelectorAll('.t706__product, .tcart__product'))
          .filter((node) => {
        ...
        domCount = productNodes.length;
        productNodes.forEach((node) => consider(node.textContent));
      }
      return { numbers, activeCount: Math.max(activeProducts.length, domCount) };
    }
    function getCommittedCartItemNumber(orderId) {
      return cartItemNumberByOrderId.get(normalizeTrackedOrderId(orderId)) || 0;
    }
    function pruneCartUploadFilesByActiveCart(products = window.tcart?.products) {
      const { numbers: activeNumbers, activeCount } = collectActiveCartItemNumbers(products);
      console.warn('[CCK-DEL] prune: активные номера в корзине =', Array.from(activeNumbers), '| activeCount =', activeCount,
        '| номера наших товаров =', Array.from(cartItemNumberByOrderId.entries()),
        '| tcart.products =', (Array.isArray(window.tcart?.products) ? window.tcart.products.map((p) => ({ name: p?.name, deleted: p?.deleted, amount: p?.amount })) : 'нет'));
      // Корзина пуста — чистим всё.
      if (!activeCount) {
        const changed = cartUploadFiles.length > 0 || cartOrderSummariesByOrderId.size > 0;
        cartUploadFiles = [];
        cartOrderSummariesByOrderId.clear();
        return { changed, activeNumbers, activeCount };
      }
      // Не удалось прочитать номера позиций — не рискуем ложными удалениями.
      if (!activeNumbers.size) {
        return { changed: false, activeNumbers, activeCount };
      }
      // Удаляем файлы товаров, которых больше нет в корзине (по номеру позиции).
      const beforeFiles = cartUploadFiles.length;
      cartUploadFiles = Array.from(cartUploadFiles || []).filter((entry) => {
        const number = getCommittedCartItemNumber(entry?.orderId);
        // Номер не определён → оставляем (не рискуем удалить нужное).
        return !number || activeNumbers.has(number);
      });
      const removedOrderIds = [];
      // Удаляем накопленные описания удалённых товаров.
      Array.from(cartOrderSummariesByOrderId.entries()).forEach(([key, entry]) => {
        if (entry?.number && !activeNumbers.has(entry.number)) {
          cartOrderSummariesByOrderId.delete(key);
          removedOrderIds.push(key);
        }
      });
      const changed = cartUploadFiles.length !== beforeFiles || removedOrderIds.length > 0;
      console.warn(`[CCK-DEL] prune результат: удалено файлов=${beforeFiles - cartUploadFiles.length}, удалено описаний=${removedOrderIds.length}, осталось файлов=${cartUploadFiles.length}, changed=${changed}`);
      if (changed) {
        try { window.__cckLog?.('cart_item_removed_pruned', { activeNumbers: Array.from(activeNumbers), removedSummaries: removedOrderIds, remainingFiles: cartUploadFiles.length }); } catch (_) {}
      }
      return { changed, activeNumbers, activeCount };
    }
        ...
        <div class="section" data-step-one-variants>
          ...
          <div class="step-1-meta-row">
            <div class="step-1-size-block">
              <div class="field-title"><span>Размер</span></div>
              <div class="size-row" style="margin-top:12px;">
                ...
                ${sizeChartImages[state.product] ? `<span class="size-chart-hint" tabindex="0" aria-label="Размерная сетка" data-size-chart="${escapeAttr(normalizeProductImageUrl(sizeChartImages[state.product]))}"><span class="size-chart-hint-icon">?</span></span>` : ''}
              </div>
            </div>
          </div>
        </div>
    ...
    async function preloadInitialRenderImages() {
      // Загружаем ВСЁ, что может понадобиться при переключении изделия / цвета /
      // шаблона, ещё до первого render(). Цель — чтобы дальнейшие переключения
      // не делали сетевых запросов и не показывали спинер.

      // 1. Картинки всех изделий (все цвета + все варианты швов).
      const allProductVariantUrls = Object.keys(productImages || {})
        .flatMap((id) => getProductVariantImageUrls(id));

      // 2. Превью карточек изделий (могут отличаться от основного варианта).
      const productCardUrls = baseProducts.map((item) => getProductCardPreviewImage(item.id));

      // 3. Размерные сетки.
      const sizeChartUrls = Object.values(sizeChartImages || {});

      // 4. Превью встроенных и пользовательских шаблонов (используются
      //    как исходник для композитного рендера на изделии).
      const templatePreviewUrls = [...templates, ...customTemplates]
        .map((tpl) => tpl?.previewImage)
        .filter(Boolean);
      // 4a. Отдельные иконки для карточек выбора (cardImage — если задано,
      //     иначе совпадает с previewImage и попадает в общий dedupe).
      const templateCardIconUrls = [...templates, ...customTemplates]
        .map((tpl) => tpl?.cardImage)
        .filter(Boolean);

      // 5. Иконки доп. элементов (шаг «Дополнительно»).
      const extrasIconUrls = (Array.isArray(extras) ? extras : [])
        .map((item) => item?.icon)
        .filter(Boolean);

      // 6. Polaroid frame (используется и в превью, и в экспорте).
      const polaroidFrameUrl = normalizeExportImageUrl(POLAROID_FRAME_IMAGE_URL);

      const dedupe = (list) => Array.from(new Set(list.filter(Boolean)));
      const rasterUrls = dedupe([
        ...allProductVariantUrls,
        ...productCardUrls,
        ...sizeChartUrls,
        ...templatePreviewUrls,
        ...templateCardIconUrls,
        ...extrasIconUrls,
      ]);
      // data-URL pipeline отдельный: Konva flat preview зовёт fetchImageAsDataUrl,
      // и без прогретого кеша каждое переключение делает отдельный fetch.
      const dataUrlSources = dedupe([
        ...allProductVariantUrls,
        ...templatePreviewUrls,
        polaroidFrameUrl,
      ]);
      ...
      await Promise.all([
        ...rasterUrls.map((url) => preloadImageUrl(url).catch(() => null)),
        ...dataUrlSources.map((url) => fetchImageAsDataUrl(url).catch(() => null)),
        konvaLoadImage(polaroidFrameUrl).catch(() => null),
      ]);
    }
    function preloadCustomTemplatePreviewImages() {
      [...templates, ...customTemplates].forEach((tpl) => {
        if (tpl.previewImage) {
          preloadImageUrl(tpl.previewImage);
          fetchImageAsDataUrl(tpl.previewImage).catch(() => {});
        }
      });
    }
    function preloadAllExternalFonts() {
      Object.keys(externalFontOptionSources).forEach((fontId) => {
        ensureExternalFontOptionReady(fontId).catch(() => {});
      });
    }
    preloadProductImages(state.product);
    preloadAllExternalFonts();
    function getPersistenceStateSnapshot() {
      ...
      return {
        ...
        fastProduction: productionMode === 'fast',
        superFastProduction: productionMode === 'superfast',
    ...
    function applyPersistedSnapshot(snapshot) {
      ...
      applyProductionMode(getProductionMode(nextState), nextState);
    ...
    // Сквозной номер позиции в корзине. Нужен, чтобы одинаковые изделия попадали
    // отдельными строками, а не склеивались в одну с количеством, но при этом БЕЗ
    // показа клиенту технического ID заказа. Номер присваивается один раз на заказ
    // (orderId) и кэшируется, чтобы не «плавал» между перерисовками href.
    const cartItemNumberByOrderId = new Map();
    function getMaxCartItemNumberFromCart() {
      let max = 0;
      const consider = (text) => {
        String(text || '').replace(/Кастомное изделие\s+(\d+)/gi, (whole, num) => {
          const parsed = parseInt(num, 10);
      ...
      if (Array.isArray(window.tcart?.products)) {
        window.tcart.products
          .filter((item) => item && item.deleted !== 'yes')
          .forEach((item) => consider(item?.name));
      }
      const cartWindow = getTildaCartWindow();
      if (cartWindow instanceof Element) {
        Array.from(cartWindow.querySelectorAll('.t706__product, .tcart__product')).forEach((node) => {
          if (node.closest('.t706__orderform, .t706 form, .t-upwidget-container, .t-input-group_uw')) return;
          consider(node.textContent);
        });
      }
      return max;
    }
    function normalizeCartItemNumberKey(orderId) {
      return normalizeTrackedOrderId(orderId) || normalizeTrackedOrderId(getFormattedOrderId(state.orderId));
    }
    function computeProspectiveCartItemNumber() {
      let maxAssigned = 0;
      cartItemNumberByOrderId.forEach((value) => {
        if (value > maxAssigned) maxAssigned = value;
      });
      // Стартуем с максимума среди уже зафиксированных в сессии и уже лежащих в корзине
      // (на случай перезагрузки страницы с сохранённой корзиной Tilda) — чтобы номер
      // был уникальным и не совпал с существующей позицией.
      return Math.max(maxAssigned, getMaxCartItemNumberFromCart()) + 1;
    }
    // Прогнозный номер БЕЗ фиксации (для предпросмотра/перерисовок href). Если номер
    // для заказа ещё не зафиксирован — показываем предполагаемый, но не «сжигаем» его.
    function getCartItemNumber(orderId) {
      const key = normalizeCartItemNumberKey(orderId);
      if (cartItemNumberByOrderId.has(key)) return cartItemNumberByOrderId.get(key);
      return computeProspectiveCartItemNumber();
    }
    // Фиксируем номер за заказом ровно один раз — вызывается при реальном оформлении.
    function commitCartItemNumber(orderId) {
      const key = normalizeCartItemNumberKey(orderId);
      if (!cartItemNumberByOrderId.has(key)) {
        cartItemNumberByOrderId.set(key, computeProspectiveCartItemNumber());
      }
      return cartItemNumberByOrderId.get(key);
    }
    function buildTildaCartTitle(payload = getOrderPayload()) {
      const itemNumber = getCartItemNumber(payload.orderId);
      const parts = [
        payload.productName,
        `Кастомное изделие ${itemNumber}`,
      ]
        .filter(Boolean)
    ...
    function normalizeTextInputValue(value = state.textValue, template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template)) {
      ...
      // Use word-wrap logic to fit text into lines, preserving full words
      const { lines } = splitPreviewTextDetailed(String(value || ''), maxChars, maxLines, { uppercase: false });
      return lines.join('\n');
    }
    function enforceTextInputLineLimits(value, template = getSelectedTemplate(), printSize = getSelectedPrintSize(undefined, template)) {
      if (isSimplePhotoTemplate(template)) return '';
      const rules = getPreviewTextRules(template, printSize);
      const maxChars = Math.max(1, Number(rules.maxCharsPerLine) || 14);
      const maxLines = Math.max(1, Number(rules.maxLines) || 5);
    ...
    function buildOrderSummaryLines(payload = getOrderPayload()) {
      return [
        ...
        `Тип изготовления: ${getProductionModeSummaryLabel(payload)}`,
        `Итого: ${formatPrice(payload.totalPrice)}`,
      ].filter(Boolean);
    }
    // Накопитель описаний по каждому товару (orderId -> { number, lines }).
    // Поле «Данные заказа» в форме Tilda одно на весь заказ, и раньше его
    // перезаписывало описание текущего товара — оставался только последний.
    // Теперь фиксируем описание каждого товара при оформлении и склеиваем все.
    const cartOrderSummariesByOrderId = new Map();
    function recordCartOrderSummary(orderId, itemNumber, payload = getOrderPayload()) {
      const key = normalizeCartItemNumberKey(orderId);
      cartOrderSummariesByOrderId.set(key, {
        number: Number(itemNumber) || (cartOrderSummariesByOrderId.size + 1),
        lines: buildOrderSummaryLines(payload),
      });
    }
    function buildCombinedOrderSummaryText() {
      const entries = Array.from(cartOrderSummariesByOrderId.values())
        .sort((a, b) => (a.number || 0) - (b.number || 0));
      if (!entries.length) return '';
      const blocks = entries.map((entry) => [`— Товар ${entry.number} —`, ...entry.lines].join('\n'));
      return [`Товаров в заказе: ${entries.length}`, '', blocks.join('\n\n')].join('\n');
    }
    // Перезаписывает поле «Данные заказа» в форме Tilda накопленной сводкой по всем
    // товарам — чтобы перерисовка корзины Tilda не «съела» описание.
    function writeCombinedOrderSummaryToTildaForm(tildaForm) {
      if (!tildaForm) return;
      const text = buildCombinedOrderSummaryText();
      if (!text) return;
      const field = ensureHiddenField(tildaForm, 'Данные заказа', 'tildaOrderSummaryField');
      if (field) field.value = text;
    }
    function updateOrderFormFields() {
      const payload = getOrderPayload();
      const summaryLines = buildOrderSummaryLines(payload);
      ...
      const tildaCommentInput = getTildaCartCommentInput(tildaForm);
      if (tildaForm) {
        tildaForm.querySelector('#tildaOrderIdField')?.remove();
        tildaForm.querySelector('#tildaOrderDataField')?.remove();
        tildaForm.querySelector('#tildaPreviewMetaField')?.remove();
        tildaForm.querySelector('#previewImageMetaField')?.remove();
        tildaForm.querySelector('#originalImageNameField')?.remove();
        tildaForm.querySelector('#originalImageTypeField')?.remove();
        tildaForm.querySelector('#uploadOrderIdField')?.remove();
        tildaForm.querySelectorAll([
          'input[name="configurator_order_id"]',
          'input[name="configurator_order_data"]',
          'input[name="configurator_preview_meta"]',
          'input[name="preview_image_meta"]',
          'input[name="original_image_name"]',
          'input[name="original_image_type"]',
          'input[name="order_id"]',
          'input[name="configurator_order_summary"]',
        ].join(', ')).forEach((el) => el.remove());
      }
      const tildaOrderSummaryField = ensureHiddenField(tildaForm, 'Данные заказа', 'tildaOrderSummaryField');
      if (tildaEmailInput) {
      ...
      if (tildaCommentInput && state.comment) {
        tildaCommentInput.value = state.comment;
        tildaCommentInput.dispatchEvent(new Event('input', { bubbles: true }));
        tildaCommentInput.dispatchEvent(new Event('change', { bubbles: true }));
      }
      // В форму Tilda пишем СВОДКУ ПО ВСЕМ товарам заказа (накопленную), а не только
      // текущему. Если ещё ничего не зафиксировано — пишем текущий товар как фолбэк.
      if (tildaOrderSummaryField) tildaOrderSummaryField.value = buildCombinedOrderSummaryText() || summaryLines.join('\n');
    ...
    function showCartUploadOverlay() {
      let overlay = document.getElementById('cartUploadOverlay');
      if (overlay) overlay.remove();
      overlay = document.createElement('div');
      overlay.id = 'cartUploadOverlay';
      overlay.innerHTML = `
        <div class="cart-upload-overlay-backdrop"></div>
        <div class="cart-upload-overlay-spinner">
          <span class="shell-loading-spinner" aria-hidden="true"></span>
          <span>Подождите</span>
        </div>
      `;
      const style = document.createElement('style');
      style.textContent = `
        #cartUploadOverlay { position: fixed; inset: 0; z-index: 2147483647; display: flex; align-items: center; justify-content: center; pointer-events: all; }
        .cart-upload-overlay-backdrop { position: absolute; inset: 0; background: rgba(255,255,255,.5); backdrop-filter: blur(2px); -webkit-backdrop-filter: blur(2px); }
        .cart-upload-overlay-spinner { position: relative; display: flex; flex-direction: column; align-items: center; gap: 12px; font-size: 14px; font-weight: 600; color: #111; background: #fff; padding: 28px 36px; border-radius: 20px; box-shadow: 0 8px 32px rgba(0,0,0,.12); }
        .cart-upload-overlay-spinner .shell-loading-spinner { width: 36px; height: 36px; border: 2.5px solid #e5e7eb; border-top-color: #111; border-radius: 50%; animation: shellLoadingSpin .7s linear infinite; }
      `;
      overlay.prepend(style);
      document.body.appendChild(overlay);
    }
    function hideCartUploadOverlay() {
      const overlay = document.getElementById('cartUploadOverlay');
      if (overlay) overlay.remove();
    }
    async function submitOrderForm(event) {
      if (orderSubmitInFlight) {
        event?.preventDefault?.();
        try { window.__cckLog?.('order_submit_blocked', { reason: 'already_in_flight' }); } catch (_) {}
        return;
      }
      if (typeof syncTildaUploadFields !== 'function' || typeof createCheckoutCompositePreviewFile !== 'function') {
        try { window.__cckLog?.('order_submit_failed', { reason: 'missing_preview_module' }); } catch (_) {}
      ...
      try {
        window.__cckLog?.('order_submit_start', {
          orderId: state.orderId,
          formattedOrderId: typeof getFormattedOrderId === 'function' ? getFormattedOrderId(state.orderId) : null,
          step: state.step,
          product: state.product,
          color: state.color,
          size: state.size,
          seam: state.seam,
          template: state.template,
          custom: state.selectedCustom,
          uploadsCount: Array.isArray(state.uploads) ? state.uploads.length : 0,
          activeCartProducts,
          hasText: Boolean(typeof getConfiguredTextValue === 'function' && getConfiguredTextValue()),
        });
      } catch (_) {}
      const currentTarget = event?.currentTarget || null;
      event?.preventDefault?.();
      try {
        // Фиксируем сквозной номер позиции (1, 2, 3...) до построения href:
        // на основе текущего количества товаров в корзине и уже зафиксированных номеров.
        const committedItemNumber = commitCartItemNumber(state.orderId);
        // Фиксируем описание ИМЕННО этого товара, чтобы поле «Данные заказа»
        // в форме Tilda содержало сводку по всем товарам, а не только последнему.
        recordCartOrderSummary(state.orderId, committedItemNumber);
        try { window.__cckLog?.('cart_item_number_committed', { orderId: state.orderId, itemNumber: committedItemNumber, summariesTotal: cartOrderSummariesByOrderId.size }); } catch (_) {}
        ...
        if (!cartOpened) {
          console.warn('Не удалось открыть корзину Tilda.');
          try { window.__cckLog?.('order_submit_failed', { reason: 'cart_open_failed', orderId: state.orderId }); } catch (_) {}
          return;
        }
        const openedCartWindow = await waitForTildaCartWindowOpen(1500);
        if (openedCartWindow) {
          ensureCartUploadCleanupObserver(openedCartWindow);
        } else if (!findTildaOrderForm()) {
          try { window.__cckLog?.('order_submit_failed', { reason: 'cart_window_not_detected', orderId: state.orderId }); } catch (_) {}
          return;
        }
        showCartUploadOverlay();
        try {
          ...
          if (openedTildaForm) {
            ...
            updateOrderFormFields();
            try { window.__cckLog?.('order_tilda_form_synced', { orderId: state.orderId }); } catch (_) {}
          } else {
            try { window.__cckLog?.('order_submit_warning', { reason: 'tilda_form_timeout', orderId: state.orderId }); } catch (_) {}
          }
        } catch (error) {
          console.warn('Не удалось передать checkout preview в upload field Tilda.', error);
          try { window.__cckLog?.('order_submit_failed', { reason: 'tilda_sync_error', orderId: state.orderId, error: String(error && error.message || error) }); } catch (_) {}
        } finally {
          hideCartUploadOverlay();
        }
        try { window.__cckLog?.('order_submit_success', { orderId: state.orderId, step: state.step }); } catch (_) {}
    ...
    function validateCheckoutContacts(showFeedback = true) {
      ...
      for (const input of requiredInputs) {
        if (!input.checkValidity()) {
          ...
          try { window.__cckLog?.('validation_contacts_invalid', { field: input.id || input.name || 'unknown', reason: input.validationMessage || 'invalid', orderId: state.orderId }); } catch (_) {}
          return false;
        }
      }
      const allFilled = Boolean(state.customerName && state.phone && state.email);
      if (!allFilled) {
        try {
          window.__cckLog?.('validation_contacts_invalid', {
            field: 'multiple',
            reason: 'empty',
            missing: [
              state.customerName ? null : 'name',
              state.phone ? null : 'phone',
    ...
    (function setupSizeChartOverlay() {
      let overlay = null;
      let overlayImg = null;
      ...
      function ensureOverlay() {
        ...
        overlay = document.createElement('div');
        overlay.className = 'size-chart-overlay';
        overlayImg = document.createElement('img');
        overlayImg.alt = 'Размерная сетка';
        overlay.appendChild(overlayImg);
        document.body.appendChild(overlay);
        return overlay;
      }
      function positionOverlay(hint) {
        if (!overlay) return;
        ...
        const down = hint.hasAttribute('data-overlay-down');
        let left = rect.right + margin;
        let top = down ? rect.bottom + margin : rect.bottom - oh;
        if (left + ow > vw - 8) left = rect.left - ow - margin;
        if (left < 8) left = Math.max(8, Math.min(vw - ow - 8, rect.left));
        if (down && top + oh > vh - 8) top = vh - oh - 8;
        if (!down && top < 8) top = 8;
      ...
      function showOverlay(hint) {
        ...
        const src = hint.getAttribute('data-size-chart');
        if (!src) return;
        ensureOverlay();
        if (overlayImg.getAttribute('src') !== src) overlayImg.setAttribute('src', src);
        currentHint = hint;
        overlay.classList.add('is-visible');
        positionOverlay(hint);
      }
      ...
      document.addEventListener('mouseover', (e) => {
        const hint = e.target.closest('.size-chart-hint');
        if (hint) showOverlay(hint);
      });
      document.addEventListener('mouseout', (e) => {
        const hint = e.target.closest('.size-chart-hint');
        if (hint && !hint.contains(e.relatedTarget)) hideOverlay();
      });
      document.addEventListener('focusin', (e) => {
        const hint = e.target.closest('.size-chart-hint');
        if (hint) showOverlay(hint);
      ...
      window.addEventListener('scroll', () => { if (currentHint) positionOverlay(currentHint); }, true);
      window.addEventListener('resize', () => { if (currentHint) positionOverlay(currentHint); });

      let modal = null;
      let modalImg = null;
      const prewarmedSizeChartSources = new Set();
      function ensureModal() {
        if (modal) return modal;
        modal = document.createElement('div');
        modal.className = 'size-chart-modal';
        modal.innerHTML = `
          <div class="size-chart-modal-backdrop"></div>
          <div class="size-chart-modal-content">
            <button type="button" class="size-chart-modal-close" aria-label="Закрыть">×</button>
            <span class="size-chart-modal-spinner" aria-hidden="true"></span>
            <img alt="Размерная сетка" />
          </div>
        `;
        ...
        modal.addEventListener('click', (ev) => {
          if (ev.target.closest('.size-chart-modal-close') || ev.target.classList.contains('size-chart-modal-backdrop')) {
      ...
      function prewarmSizeChartImage(src) {
        if (!src || prewarmedSizeChartSources.has(src)) return;
        prewarmedSizeChartSources.add(src);
        const warmImg = new Image();
        warmImg.decoding = 'async';
        warmImg.src = src;
        if (typeof warmImg.decode === 'function') {
          warmImg.decode().catch(() => {});
        }
      }
      function setModalImageSrc(src) {
        ensureModal();
        if (modalImg.getAttribute('src') === src) return;
        if (modalImg.complete && modalImg.naturalWidth > 0 && modalImg.getAttribute('src')) {
          modal.classList.remove('is-loading');
        } else {
      ...
      function prewarmMobileSizeChart() {
        const hint = document.querySelector('.size-chart-hint');
        if (!hint) return;
        const src = hint.getAttribute('data-size-chart');
        if (!src) return;
        prewarmSizeChartImage(src);
        setModalImageSrc(src);
      }
      ensureModal();
      Object.values((typeof sizeChartImages === 'object' && sizeChartImages) || {}).forEach((url) => {
        if (!url) return;
        const normalized = typeof normalizeProductImageUrl === 'function' ? normalizeProductImageUrl(url) : url;
        prewarmSizeChartImage(normalized);
      });
      const sizeChartObserver = new MutationObserver(() => prewarmMobileSizeChart());
      sizeChartObserver.observe(document.body, {
        childList: true,
        subtree: true,
        attributes: true,
        attributeFilter: ['data-size-chart'],
      });
      prewarmMobileSizeChart();
      document.addEventListener('click', (ev) => {
        const hint = ev.target.closest('.size-chart-hint');
        if (!hint) return;
        if (!window.matchMedia('(max-width: 768px)').matches) return;
        const src = hint.getAttribute('data-size-chart');
        if (!src) return;
      ...
      document.addEventListener('keydown', (ev) => {
        if (ev.key === 'Escape' && modal && modal.classList.contains('is-visible')) {
          modal.classList.remove('is-visible');
        }
      });
    })();
    document.addEventListener('click', (e) => {
      const goStep2PhotoTrigger = e.target.closest('[data-go-step-2-photo]');
      if (goStep2PhotoTrigger) {
        closeModal('uploadBlockedModal');
        mobileStepNavScrollPending = true;
        state.step = 2;
        render();
        return;
    ...
    function nextStep() {
      if (!isStepComplete(state.step)) {
        try { window.__cckLog?.('step_next_blocked', { step: state.step, reason: 'step_incomplete', orderId: state.orderId }); } catch (_) {}
        return;
      }
      ...
      try { window.__cckLog?.('step_changed', { from, to: state.step, direction: 'next', skipped: next !== from + 1, orderId: state.orderId }); } catch (_) {}
      render();
    }
    function prevStep() {
      const from = state.step;
      ...
      try { window.__cckLog?.('step_changed', { from, to: state.step, direction: 'prev', skipped: prev !== from - 1, orderId: state.orderId }); } catch (_) {}
      render();
    }
    ...
    function isMobilePreviewInspectEnabled() { return false; }
    function ensurePreviewGestureHintElement() { return null; }
    ...
    async function handleFiles(fileList, slotIndex = 0) {
      const incomingFiles = Array.from(fileList || []);
      const rawFiles = incomingFiles.filter((file) => isSupportedImageFile(file));
      try {
        window.__cckLog?.('files_received', {
          slotIndex,
          incoming: incomingFiles.length,
          accepted: rawFiles.length,
          rejected: incomingFiles.length - rawFiles.length,
          totalSize: rawFiles.reduce((s, f) => s + (f.size || 0), 0),
          types: rawFiles.map((f) => f.type || 'unknown'),
          hasHeic: rawFiles.some((f) => /(^|\W)hei[cf]($|\W)/i.test(f.type || '') || /\.hei[cf]$/i.test(f.name || '')),
          orderId: state.orderId,
        });
      } catch (_) {}
      if (!rawFiles.length) {
        try { window.__cckLog?.('files_rejected', { slotIndex, incoming: incomingFiles.length, reason: 'unsupported_format', orderId: state.orderId }); } catch (_) {}
        return;
      }

      // Если есть HEIC — показываем спиннер «Декодируем фото», т.к. конвертация
      // занимает 1-3 секунды на большом снимке и без индикатора кажется, что
      // конструктор завис. Используем отдельный класс is-heic-decoding —
      // обычные switch-color/template спиннеры остаются отключёнными.
      const hasHeic = rawFiles.some(isHeicFile);
      const garmentEl = document.getElementById('garment');
      const overlayTextEl = document.querySelector('#garmentLoadingOverlay .garment-loading-text');
      let savedOverlayText = '';
      if (hasHeic) {
        if (overlayTextEl) {
          savedOverlayText = overlayTextEl.textContent || '';
          overlayTextEl.textContent = 'Декодируем фото';
        }
        if (garmentEl) garmentEl.classList.add('is-heic-decoding');
      }

      let files;
      try {
        // Конвертируем HEIC → JPEG ДО createObjectURL, иначе <img src="blob:...heic">
        // не отрисуется в Chrome/Firefox (нативно HEIC поддерживает только Safari).
        files = await Promise.all(rawFiles.map((file) => normalizeUploadForPreview(file)));
        files = files.filter(Boolean);
      } finally {
        if (hasHeic) {
          if (garmentEl) garmentEl.classList.remove('is-heic-decoding');
          if (overlayTextEl && savedOverlayText) overlayTextEl.textContent = savedOverlayText;
        }
      }
      ...
      __resetCropZoomAllowed(1); // явный сброс при загрузке нового фото
    ...
    function removeUploadAt(index = 0) {
      ...
      if (!state.uploads.length) {
        __resetCropZoomAllowed(1); // явный сброс при удалении всех фото
        state.cropPosition = { x: 0, y: 0 };
        state.cropRotation = 0;
...
// ===== constructor-checkout-mvp-tilda-js-1-preview.html =====
    ...
    async function fetchImageAsDataUrl(url) {
      ...
      const resolvedPromise = promise.then((dataUrl) => {
        if (dataUrl && dataUrl.startsWith('data:')) {
          resolvedImageDataUrls.set(normalizedUrl, dataUrl);
        }
        return dataUrl;
      });
      exportImageDataUrlPromises.set(normalizedUrl, resolvedPromise);
    ...
    function ensureHeic2anyReady() {
      ...
      heic2anyLoadPromise = new Promise((resolve, reject) => {
        ...
        script.onload = () => {
          ...
          else reject(new Error('heic2any не зарегистрировался в window'));
        };
        script.onerror = () => reject(new Error('Не удалось загрузить heic2any'));
        document.head.appendChild(script);
      }).catch((err) => {
        heic2anyLoadPromise = null; // позволяем повторить попытку
        throw err;
      });
      return heic2anyLoadPromise;
    }

    async function convertHeicFileToJpeg(file, quality = 0.92) {
      if (!file) return null;
      try {
        ...
        console.warn('Не удалось декодировать HEIC через heic2any:', err);
        return null;
      }
    }

    async function normalizeUploadForPreview(file) {
      // Конвертирует форматы, которые браузер не умеет рендерить в <img>
      // (HEIC/HEIF на Chrome/Firefox), в JPEG ДО создания blob-URL для превью.
      // AVIF поддерживается современными браузерами нативно — не трогаем.
      if (!file) return file;
      if (isHeicFile(file)) {
    ...
    async function normalizeFileForTildaUpload(file) {
      ...
      // HEIC обрабатываем специальным декодером — браузерный <img> его не читает.
      if (isHeicFile(file)) {
        const heicConverted = await convertHeicFileToJpeg(file);
        if (heicConverted) return heicConverted;
      }
    ...
    async function createCheckoutCompositePreviewUrl(options = {}) {
      ...
      if (state.selectedCustom) {
        const customTpl = getSelectedCustomTemplate();
        if (customTpl && customTpl.previewImage) {
          const tplDefaults = customTpl.photoDefaults || null;
          const spConfig = KONVA_TEMPLATE_CONFIGS['simple-photo'];
          const spDefaults = spConfig?.photoDefaults || {};
          const spScale = spDefaults.scale || 0.8;
          const cpW = tplDefaults?.width ?? (110 * spScale);
          const cpH = tplDefaults?.height ?? (138 * spScale);
          const cpX = tplDefaults?.exportX ?? spDefaults.exportX ?? (1000 - cpW) / 2;
          const cpY = tplDefaults?.exportY ?? spDefaults.exportY ?? 483;
          const customImgHref = embedAssets ? await fetchImageAsDataUrl(customTpl.previewImage) : customTpl.previewImage;
          designMarkup = `
            <image
              href="${escapeSvg(customImgHref)}"
    ...
    async function renderCheckoutCompositePreviewCanvas(canvas) {
      ...
      if (state.selectedCustom || !selectedTemplate) {
        if (state.selectedCustom) {
          ...
          if (customTpl && customTpl.previewImage) {
            try {
              const cpDataUrl = await fetchImageAsDataUrl(customTpl.previewImage);
              const cpImg = await loadImageElement(cpDataUrl);
              const tplDefaults = customTpl.photoDefaults || null;
              const spConfig = KONVA_TEMPLATE_CONFIGS['simple-photo'];
              const spDefaults = spConfig?.photoDefaults || {};
              const spScale = spDefaults.scale || 0.8;
              ...
              const cpX = tplDefaults?.exportX ?? spDefaults.exportX ?? (1000 - cpW) / 2;
              const cpY = tplDefaults?.exportY ?? spDefaults.exportY ?? 483;
              const nw = cpImg.naturalWidth || cpImg.width;
              const nh = cpImg.naturalHeight || cpImg.height;
              const fitS = Math.min(cpW / nw, cpH / nh);
              const dw = nw * fitS, dh = nh * fitS;
              ctx.drawImage(cpImg, cpX + (cpW - dw) / 2, cpY + (cpH - dh) / 2, dw, dh);
            } catch (_) {}
          }
        }
        ctx.restore();
      ...
      if (templatePhotoLayout) {
        ...
        if (konvaConfig?.isPolaroid) {
          ...
          if (polaroidFrameSource) {
            try {
              const frameImg = await loadImageElement(polaroidFrameSource);
              if (frameImg) {
                const nW = frameImg.naturalWidth || frameImg.width || fW;
                const nH = frameImg.naturalHeight || frameImg.height || fH;
                const s = Math.min(fW / nW, fH / nH);
                ctx.drawImage(frameImg, fX, fY, nW * s, nH * s);
              }
            } catch (_) {}
          }
        }
      ...
      if (isKonvaAvailable()) {
        try {
          ...
          const frameDataUrl = sceneParams.frameSrc
            ...
            : null;
          const customPreviewDataUrl = sceneParams.customPreviewSrc
            ? await fetchImageAsDataUrl(sceneParams.customPreviewSrc)
            : null;
          const exportParams = {
            ...sceneParams,
            baseImageSrc: baseImageDataUrl || sceneParams.baseImageSrc,
            frameSrc: frameDataUrl || sceneParams.frameSrc,
            customPreviewSrc: customPreviewDataUrl || sceneParams.customPreviewSrc,
          };
    ...
    async function syncTildaUploadFields(tildaForm) {
      ...
      try {
        window.__cckLog?.('cart_upload_merge', {
          orderId: state.orderId,
          formattedOrderId: getFormattedOrderId(state.orderId),
          currentOrderFiles: currentOrderFiles.map((file) => file?.name || '(без имени)'),
          trackedTotal: filesForTilda.length,
          trackedNames: filesForTilda.map((file) => file?.name || '(без имени)'),
          customMode,
        });
      } catch (_) {}
      ...
      await withPreservedTildaCartFormValues(async () => {
        const fileNames = filesForTilda.map((file) => file?.name || '(без имени)');
        const upwidgetContainer = await waitForTildaUpwidgetContainer(tildaForm, 5000);
        if (upwidgetContainer && filesForTilda.length) {
          // Каждый файл заливается в CDN один раз (кэш), затем виджет
          // перерисовывается ПОЛНЫМ набором из кэша. Уже загруженные файлы
          // (в т.ч. оригиналы предыдущих товаров) не перезаливаются и не теряются.
          const { rendered, attached, total, failed } = await attachTrackedCartFilesToUpwidget(upwidgetContainer);
          try { window.__cckLog?.('cart_upload_attach', { orderId: state.orderId, attached, total, failed }); } catch (_) {}
          if (failed.length) {
            console.error('[constructor] Часть файлов не удалось прикрепить к корзине Tilda:', failed);
          }
          if (rendered) {
            return;
          }
          console.warn('[constructor] attachTrackedCartFilesToUpwidget не отрисовал файлы, пробуем input[type=file] фолбэк.');
        } else if (!filesForTilda.length) {
          // нет файлов для прикрепления
        } else {
          console.warn('[constructor] Не найден .t-upwidget-container в форме Tilda, пробуем input[type=file].');
        }
        const fileInputs = Array.from(tildaForm.querySelectorAll('.t-input-group_uw input[type="file"], input[type="file"]'));
        if (!fileInputs.length) {
          if (filesForTilda.length) {
            console.warn('[constructor] В форме Tilda нет input[type=file] — файлы не прикреплены.', fileNames);
          }
          return;
        }
        const synced = syncFilesToAvailableInputs(fileInputs, filesForTilda);
        if (!synced) {
          console.warn('[constructor] Не удалось прикрепить файлы через input[type=file].', fileNames);
        }
      }, {
        form: tildaForm,
          ...
          <div class="section">
            ...
            <div class="template-choice-mobile-group">
              <div class="template-choice-header">
                <div class="subtext">${stepTwoTab === 'custom'
                  ? 'Для данных шаблонов визуализация изделия недоступна — дизайнер поработает над вашим дизайном индивидуально.'
                  : 'Доступна визуализация с вашим фото прямо сейчас!'}</div>
              </div>
              <div class="template-choice-list template-choice-mobile-list" data-preserve-scroll-key="mobile-templates-${escapeAttr(stepTwoTab)}">
                ${mobileTemplateCards}
                ${stepTwoTab === 'custom' ? renderCustomContactCard() : ''}
              </div>
            </div>
            ...
            <div class="template-choice-group">
              <div class="template-choice-header">
                <div class="subhead">Индивидуальная кастомизация</div>
                <div class="subtext">Для данных шаблонов визуализация изделия недоступна — дизайнер поработает над вашим дизайном индивидуально.</div>
              </div>
              <div class="template-choice-list">
                  ...
                  return `
                    <label class="custom-upload-trigger">
                      ...
                      <span class="custom-upload-trigger-copy">
                        <span class="custom-upload-trigger-title">${upload ? 'Заменить: ' + escapeHtml(upload.name) : 'Загрузите изображение'}</span>
                        <span class="custom-upload-trigger-text">JPG / PNG — или перетащите на превью слева</span>
                      </span>
                    </label>
                  `;
                }).join('')}
            ...
            ${state.selectedCustom ? `
              <div class="custom-upload-shell">
                <div class="custom-upload-head">
                  <div>
                    <div class="subtext">Загрузите до ${MAX_CUSTOM_UPLOADS} фотографий. Мы передадим их дизайнеру для создания индивидуального дизайна. Для данных шаблонов визуализация изделия недоступна - дизайнер индивидуально проработает ваш дизайн и пришлет на согласование.</div>
                  </div>
          ...
          <div class="section">
            <div>
              ...
              <div class="phrase-dropdown" data-step-dropdown>
                <button class="dropdown-toggle" data-dropdown-toggle type="button" aria-expanded="false">Готовые фразы для принта</button>
            ...
            <div>
              <div class="field-title">Выберите размер текста</div>
          ...
          <div class="section">
            <div class="stack">
              ${extras.map((e) => {
                ...
                const isToggle = e.id === 'hand-drawn';
                const controls = isToggle
                  ? `
                        <button type="button" class="switch ${count > 0 ? 'active' : ''}" data-toggle="${e.id}" aria-pressed="${count > 0 ? 'true' : 'false'}">
                          <span class="switch-knob"></span>
                        </button>
                        <div class="extra-card-price">${formatPrice(e.price)}</div>
                      `
                  : `
                        <button type="button" class="icon-btn extra-counter-btn" data-minus="${e.id}">−</button>
                        <div class="extra-counter-value">${count}</div>
                        <button type="button" class="icon-btn extra-counter-btn" data-plus="${e.id}">+</button>
                        <div class="extra-card-price">${formatPrice(e.price)}</div>
                      `;
                return `
                  <div class="extra-card ${count > 0 ? 'active' : ''}">
                    <div class="extra-card-head">
                      <div class="extra-card-title">${e.icon ? `<img class="extra-card-icon" src="${escapeAttr(normalizeProductImageUrl(e.icon))}" alt="" aria-hidden="true" loading="eager" decoding="async" referrerpolicy="no-referrer" onerror="this.style.visibility='hidden'">` : ''}<span class="extra-card-title-text">${e.name}</span>${e.id === 'hand-drawn' ? ' <span class="size-chart-hint" tabindex="0" aria-label="Пример рисованных элементов" data-overlay-down data-size-chart="https://optim.tildacdn.com/tild6338-6139-4536-b835-333062356566/-/format/webp/_.jpg"><span class="size-chart-hint-icon">?</span></span>' : ''}</div>
                      <div class="extra-card-controls">
                        ${controls}
                      </div>
                    </div>
                    <div class="extra-card-description subtext">${e.description || 'Можно добавить к покупке'}</div>
                  </div>
                `;
              }).join('')}
            </div>
      ...
      if (state.step === 6) {
        ...
        const reviewStage = `
          <div class="checkout-review-grid">
            ...
            <div class="info-card production-card">
              <div style="display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap;">
                <div class="subhead" style="font-size:22px; margin:0;">Изготовление</div>
                <div class="status-pill ${productionMode !== 'standard' ? 'ok' : ''}" style="margin:0;">${getProductionModeStatusLabel()}</div>
              </div>
              <div class="stack production-options" style="margin-top:16px;">
                <button class="toggle-card production-option-card ${productionMode === 'standard' ? 'active' : ''}" data-production-mode="standard" type="button" aria-pressed="${productionMode === 'standard'}">
                  <div class="production-option-main">
                    <div class="production-option-title">Стандарт</div>
                    <div class="subtext">Срок изготовления 4-6 дней</div>
                  </div>
                </button>

                <button class="toggle-card production-option-card ${productionMode === 'fast' ? 'active' : ''}" data-production-mode="fast" type="button" aria-pressed="${productionMode === 'fast'}">
                  <div class="production-option-main">
                    <div class="production-option-title">Ускоренный</div>
                    <div class="subtext">Срок изготовления 2-3 дня</div>
                  </div>
                  <div class="production-card-price production-option-price">+ ${formatPrice(500)}</div>
                </button>

                <button class="toggle-card production-option-card ${productionMode === 'superfast' ? 'active' : ''}" data-production-mode="superfast" type="button" aria-pressed="${productionMode === 'superfast'}">
                  <div class="production-option-main">
                    <div class="production-option-title">За 24 часа</div>
                    <div class="subtext">Доступно при оформлении заказа до 17:00 по Москве</div>
                  </div>
                  <div class="production-card-price production-option-price">+ ${formatPrice(800)}</div>
                </button>
              </div>
              <div class="production-comment">
                <label class="production-comment-label" for="configuratorComment">Комментарий</label>
                <textarea id="configuratorComment" class="production-comment-input" rows="2" placeholder="Пожелания к заказу">${escapeHtml(state.comment || '')}</textarea>
              </div>
            </div>
          </div>
          <div class="checkout-stage-actions">
            ...
            <a class="btn primary" data-submit-order href="#order:1) Тест =1">Добавить в корзину</a>
          </div>
    ...
    function bindStepEvents() {
      ...
      if (state.step === 1) {
        ...
        container.querySelectorAll('[data-color]').forEach((btn) => {
          btn.onclick = () => {
            const sigBefore = stepOneVariantSetSignature();
            state.color = btn.dataset.color;
            syncTextColorToGarment();
            syncProductVariantState();
            // Если набор доступных швов/размеров не изменился — пропускаем
            // тяжёлую пересборку variantsBlock и работаем по lightweight-пути.
            if (stepOneVariantSetSignature() === sigBefore) {
              updateStepOneActiveSelectionOnly();
            } else {
              updateStepOneProductSelection();
            }
          };
      ...
      if (state.step === 2) {
        ...
        container.querySelectorAll('[data-template]').forEach((btn) => {
          btn.onclick = () => {
            const wasOnTemplateTab = state.stepTwoTab === 'template';
            state.stepTwoTab = 'template';
            state.template = btn.dataset.template;
            state.selectedCustom = null;
            normalizeTextCustomizationState();
            if (wasOnTemplateTab) {
              // Lightweight: только active-класс + перерисовка превью.
              // Не вызываем render() — иначе stepPanel.innerHTML
              // пересоздаётся, и <img> карточек шаблонов пере-decode'ятся
              // («рендерятся по новой»).
              updateStepTwoActiveSelectionOnly();
            } else {
              renderPreservingStepContentScroll();
            }
          };
        ...
        container.querySelectorAll('[data-custom]').forEach((btn) => {
          btn.onclick = () => {
            const wasOnCustomTab = state.stepTwoTab === 'custom';
      ...
      if (state.step === 3) {
        ...
        if (garmentWrap) {
          ...
          // Throttled live re-render для production-режима. updatePreviewImageStyles
          // меняет только legacy DOM-overlay (#designLayer .crop-image), который
          // в Konva-production-mode скрыт. Чтобы пользователь видел перемещение
          // фото на ОТРИСОВАННОМ flatPreview, запускаем renderVisualizer() c
          // rAF-throttling: не чаще одного re-render за кадр, токен-система
          // konvaFlatPreviewRenderToken отменит устаревшие рендеры.
          let liveRenderRaf = null;
          const requestLivePreviewRefresh = () => {
            if (liveRenderRaf) return;
            liveRenderRaf = requestAnimationFrame(() => {
              liveRenderRaf = null;
              try { renderVisualizer(); } catch (_) {}
            });
          };
      ...
      if (state.step === 4) {
        ...
        if (textInput) {
          ...
          textInput.oninput = (e) => {
            ...
            const maxChars = Math.max(1, Number(rules.maxCharsPerLine) || 14);
            const maxLines = Math.max(1, Number(rules.maxLines) || 5);
            const previousValue = String(state.textValue || '');
            const lowercaseOnly = isFontLowercaseOnly(state.font, template);
            let rawValue = isSimplePhotoTemplate(template) ? '' : String(e.target.value);
            if (lowercaseOnly) rawValue = rawValue.toLowerCase();
            const newLines = rawValue.replace(/\r\n?/g, '\n').split('\n');
            const lineExceeded = newLines.some((line) => line.length > maxChars);
            const tooManyLines = newLines.length > maxLines;
            const isNetInsert = rawValue.length > previousValue.length;
            if ((lineExceeded || tooManyLines) && isNetInsert) {
              const cursorPos = Math.max(0, e.target.selectionStart - (rawValue.length - previousValue.length));
        ...
        container.querySelectorAll('[data-phrase]').forEach((btn) => {
          btn.onclick = () => {
            ...
            const phraseValue = String(btn.dataset.phrase || '');
            state.textValue = isFontLowercaseOnly(state.font, selectedTemplate)
              ? phraseValue.toLowerCase()
              : phraseValue;
        ...
        container.querySelectorAll('[data-text-color]').forEach((btn) => {
          btn.onclick = () => {
            // Защита от сброса зума/позиции (см. textInput.oninput выше).
            const savedCropZoom = state.cropZoom;
            const savedCropPosition = { ...state.cropPosition };
            state.textColor = btn.dataset.textColor;
            updateStepFourControls();
            if (state.cropZoom !== savedCropZoom) state.cropZoom = savedCropZoom;
            if (state.cropPosition?.x !== savedCropPosition.x || state.cropPosition?.y !== savedCropPosition.y) {
        ...
        container.querySelectorAll('[data-font-value]').forEach((btn) => {
          btn.onclick = async () => {
            ...
            if (isFontLowercaseOnly(state.font, selectedTemplate) && state.textValue) {
              state.textValue = String(state.textValue).toLowerCase();
            }
            try { await ensureSelectedPreviewFontReady(selectedTemplate); } catch (_) {}
            updateStepFourControls({ syncTextInput: true });
            if (state.cropZoom !== savedCropZoom) state.cropZoom = savedCropZoom;
      ...
      if (state.step === 5) {
        ...
        container.querySelectorAll('[data-toggle]').forEach((btn) => {
          btn.onclick = () => {
            ...
            const current = extrasCount[id] || 0;
            changeExtraCount(id, current > 0 ? -current : 1);
          };
        });
      }
      ...
      if (state.step === 6) {
        ...
        document.querySelectorAll('[data-production-mode]').forEach((btn) => {
          btn.addEventListener('click', () => {
            const nextMode = btn.dataset.productionMode || 'standard';
            if (getProductionMode() === nextMode) return;
            applyProductionMode(nextMode);
            render();
          });
        });
        const emailInput = document.getElementById('configuratorEmail');
...
function getTildaCartForm() {
    ...
    async function uploadFilesToTildaUpwidget(container, files, options = {}) {
      ...
      for (const file of normalizedFiles.slice(0, 10)) {
        // Крупные оригиналы иногда падают на одной попытке (таймаут/сеть) и раньше
        // молча терялись. Делаем несколько попыток, прежде чем сдаться.
        let uploadedItem = null;
        let lastError = null;
        for (let attempt = 1; attempt <= 3 && !uploadedItem; attempt += 1) {
          try {
            uploadedItem = await uploadFileToTildaUpwidgetApi(container, file);
          } catch (error) {
            lastError = error;
            try { window.__cckLog?.('cart_upload_api_retry', { file: file?.name || '', attempt, error: String(error && error.message || error) }); } catch (_) {}
            console.warn(`[constructor] Не удалось загрузить файл в Tilda upwidget (попытка ${attempt}/3), повторяю.`, file?.name || '', error);
        ...
        if (uploadedItem) {
          ...
          failedFiles.push(file?.name || '(без имени)');
        }
      }
      if (failedFiles.length) {
        try { window.__cckLog?.('cart_upload_api_failed', { files: failedFiles }); } catch (_) {}
        console.error('[constructor] Не удалось загрузить файлы в Tilda upwidget после повторов:', failedFiles);
      }
    ...
    function syncFileToInput(input, files) {
      ...
      try {
        ...
        console.warn('Не удалось синхронизировать файлы с input[type=file]', error);
        return false;
      }
    }
    ...
    // Стабильный ключ файла, устойчивый к расширению и суффиксу Tilda CDN
    // (Tilda дописывает к загруженному файлу _tildaNNNN). И наше имя
    // `id_3687860678_orig.jpg`, и вернувшееся `id_3687860678_orig_tilda54213179.jpg`
    // дают один ключ `id_3687860678_orig`, поэтому уже загруженный файл корректно
    // распознаётся как «уже есть» и не перезаливается.
    function getCartUploadFileKey(name = '') {
      const match = String(name || '').match(/id_\d+_(?:orig|preview)(?:_\d+)?/i);
      return match ? match[0].toLowerCase() : String(name || '').trim().toLowerCase();
    }
    function getTildaFormUploadedFileNames(tildaForm) {
      if (!tildaForm) return [];
      const container = getTildaUpwidgetContainer(tildaForm);
      if (container) {
        return getTildaUpwidgetExistingEntries(container)
          .map((entry) => entry?.sourceFile?.name || entry?.item?.uploadedName || extractTildaUploadedFileName(entry?.item?.value || ''))
          .filter(Boolean);
      }
      const fileInputs = Array.from(tildaForm.querySelectorAll('.t-input-group_uw input[type="file"], input[type="file"]'));
      const names = [];
    ...
    function getTildaFormUploadedFileKeys(tildaForm) {
      return new Set(getTildaFormUploadedFileNames(tildaForm).map((name) => getCartUploadFileKey(name)));
    }
    function getMissingTrackedCartUploadFiles(tildaForm) {
      const trackedFiles = getCartUploadFilesSnapshot();
      if (!trackedFiles.length) return [];
      const presentKeys = getTildaFormUploadedFileKeys(tildaForm);
      return trackedFiles.filter((file) => file?.name && !presentKeys.has(getCartUploadFileKey(file.name)));
    }
    // Кэш уже загруженных в Tilda CDN файлов: ключ файла -> { value (URL), uploadedName, sourceName }.
    // Каждый файл заливаем в CDN РОВНО ОДИН РАЗ. Дальше форму корзины всегда
    // перерисовываем из этого кэша (без сетевых повторов) — поэтому успешно
    // загруженный файл уже не может «потеряться» при перерисовке корзины Tilda,
    // и мы не перезаливаем (и не роняем повторно) то, что уже загружено.
    const uploadedCdnFilesByKey = new Map();
    const cdnUploadAttemptsByKey = new Map();
    function isForbiddenFileTypeError(error) {
      const msg = String(error && error.message || error || '').toLowerCase();
      return /запрещ|forbidden|тип файла|file type|недопустим/.test(msg);
    }
    // Заливает конкретный файл с сетевыми повторами. Возвращает {uploaded, forbidden}.
    // forbidden=true означает «Tilda отклонила тип» — повторять те же байты бесполезно,
    // нужно переконвертировать в чистый JPEG.
    async function uploadCandidateToCdnWithRetries(container, key, candidate, maxAttempts = 2) {
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        cdnUploadAttemptsByKey.set(key, (cdnUploadAttemptsByKey.get(key) || 0) + 1);
        try {
          const uploaded = await uploadFileToTildaUpwidgetApi(container, candidate);
          if (uploaded) return { uploaded, forbidden: false };
        } catch (error) {
          const msg = String(error && error.message || error);
          try { window.__cckLog?.('cart_upload_api_retry', { file: candidate.name, attempt, error: msg }); } catch (_) {}
          if (isForbiddenFileTypeError(error)) return { uploaded: null, forbidden: true };
          if (attempt < maxAttempts) await delay(300 * attempt);
        }
      }
      return { uploaded: null, forbidden: false };
    }
    async function ensureCartFileUploadedToCdn(container, file) {
      if (!container || !file?.name) return null;
      const key = getCartUploadFileKey(file.name);
      if (uploadedCdnFilesByKey.has(key)) {
        return uploadedCdnFilesByKey.get(key);
      }
      if ((cdnUploadAttemptsByKey.get(key) || 0) >= 9) {
        return null;
      }
      let { uploaded, forbidden } = await uploadCandidateToCdnWithRetries(container, key, file);
      // Tilda отклонила тип (или заливка не удалась) — переконвертируем оригинал в
      // чистый JPEG через canvas (как превью, которое всегда проходит) и пробуем снова.
      if (!uploaded) {
        try { window.__cckLog?.('cart_upload_reencode', { file: file.name, reason: forbidden ? 'forbidden_type' : 'upload_failed' }); } catch (_) {}
        const jpeg = await convertImageFileToJpeg(file).catch(() => null);
        if (jpeg) {
          const normalized = cloneFileWithName(jpeg, file.name, 'image/jpeg');
          ({ uploaded } = await uploadCandidateToCdnWithRetries(container, key, normalized));
        }
      }
      if (uploaded) {
        const entry = { value: uploaded.value, uploadedName: uploaded.uploadedName, sourceName: file.name };
        uploadedCdnFilesByKey.set(key, entry);
        return entry;
      }
      return null;
    }
    // Заливает все отслеживаемые файлы (каждый по одному разу) и перерисовывает виджет
    // ПОЛНЫМ набором из кэша. Идемпотентно — вызывать можно сколько угодно раз.
    async function attachTrackedCartFilesToUpwidget(container) {
      const trackedFiles = getCartUploadFilesSnapshot();
      if (!container) {
        return { rendered: false, attached: 0, total: trackedFiles.length, failed: trackedFiles.map((file) => file?.name || '') };
      }
      const items = [];
      const sources = [];
      const failed = [];
      for (const file of trackedFiles.slice(0, 10)) {
        const entry = await ensureCartFileUploadedToCdn(container, file);
        if (entry) {
          items.push({ value: entry.value, uploadedName: entry.uploadedName });
          sources.push({ name: file.name });
        } else {
          failed.push(file?.name || '(без имени)');
        }
      }
      const rendered = items.length ? renderTildaUpwidgetUploadedState(container, items, sources) : false;
      if (failed.length) {
        try { window.__cckLog?.('cart_upload_api_failed', { files: failed }); } catch (_) {}
        console.error('[constructor] Не удалось загрузить файлы в Tilda CDN после повторов:', failed);
      }
      return { rendered, attached: items.length, total: trackedFiles.length, failed };
    }
    async function syncCartUploadFilesWithActiveCart() {
      if (cartUploadCleanupInFlight) {
        console.warn('[CCK-DEL] cleanup пропущен — уже выполняется.');
        return false;
      }
      cartUploadCleanupInFlight = true;
      try {
        console.warn('[CCK-DEL] cleanup START (синхронизация состава корзины).');
        const { changed, activeCount } = pruneCartUploadFilesByActiveCart();
        const targetForm = findTildaOrderForm({ requireFileInput: true })
          || await waitForTildaOrderForm(1500, { requireFileInput: true });
        if (!targetForm) {
          console.warn('[CCK-DEL] cleanup: форма заказа Tilda НЕ найдена — выходим (форма не обновлена).');
          return false;
        }
        const trackedFiles = getCartUploadFilesSnapshot();
        const preservedFormValues = captureTildaCartFormValues(targetForm);
        if (!trackedFiles.length) {
          // У нас нет своих файлов. Чистим виджет ТОЛЬКО если корзина реально пуста.
          // Иначе (напр. перезагрузка страницы с сохранённой корзиной Tilda) не трогаем
          // виджет, чтобы не стереть уже прикреплённые файлы.
          if (activeCount !== 0) {
            console.warn('[CCK-DEL] tracked пуст, но в корзине есть товары — виджет не трогаю.');
            return false;
          }
          uploadedCdnFilesByKey.clear();
          cdnUploadAttemptsByKey.clear();
          cartOrderSummariesByOrderId.clear();
          cartItemNumberByOrderId.clear();
          return await withPreservedTildaCartFormValues(async () => {
            const container = await waitForTildaUpwidgetContainer(targetForm, 1000);
            if (container) { await clearTildaUpwidgetFiles(container); return true; }
            Array.from(targetForm.querySelectorAll('.t-input-group_uw input[type="file"], input[type="file"]')).forEach((input) => clearFilesFromInput(input));
            return true;
          }, { form: targetForm, values: preservedFormValues });
        }
        // Поддерживаем актуальной сводку «Данные заказа» по всем товарам — в т.ч. после
        // удаления товара из корзины (его описание уже удалено в prune).
        writeCombinedOrderSummaryToTildaForm(targetForm);
        // Решаем, нужна ли пересборка файлов виджета:
        //  - не хватает файлов (Tilda выбросила при перерисовке), или
        //  - в виджете остались «лишние» файлы удалённого товара (changed / hasStale).
        const missingCount = getMissingTrackedCartUploadFiles(targetForm).length;
        const trackedKeys = new Set(trackedFiles.map((file) => getCartUploadFileKey(file.name)));
        const widgetKeysNow = Array.from(getTildaFormUploadedFileKeys(targetForm));
        const hasStaleInWidget = widgetKeysNow.some((widgetKey) => !trackedKeys.has(widgetKey));
        console.warn(`[CCK-DEL] cleanup: changed=${changed}, файлов в наборе=${trackedFiles.length}, в виджете=${widgetKeysNow.length}, не хватает=${missingCount}, лишних(stale)=${hasStaleInWidget}`,
          '| виджет:', widgetKeysNow, '| набор:', Array.from(trackedKeys));
        if (!changed && !missingCount && !hasStaleInWidget) {
          console.warn('[CCK-DEL] cleanup: всё совпадает, ничего не делаю.');
          return false;
        }
        // Если в форме есть «лишние» файлы (удалили товар) — нужно их вычистить, в т.ч.
        // из модели Tilda, а не просто дорисовать недостающие.
        const needsHardClear = changed || hasStaleInWidget;
        if (changed) {
          // Подстраховка от гонок: Tilda могла прочитаться в полу-обновлённом состоянии
          // или перерисовать корзину и вернуть свои файлы — через пару секунд
          // перепроверяем (одноразово на каждое фактическое удаление).
          window.setTimeout(() => scheduleCartUploadCleanup(0), 1800);
        }
        return await withPreservedTildaCartFormValues(async () => {
          const container = await waitForTildaUpwidgetContainer(targetForm, 1500);
          if (container) {
            // При удалении товара сначала ПОЛНОСТЬЮ очищаем виджет (механизмом Tilda),
            // чтобы убрать файлы удалённого товара и из модели Tilda, затем заново
            // отрисовываем оставшиеся из кэша (без повторной заливки).
            if (needsHardClear) {
              await clearTildaUpwidgetFiles(container);
            }
            const { rendered, attached, total, failed } = await attachTrackedCartFilesToUpwidget(container);
            try { window.__cckLog?.('cart_upload_reconciled', { attached, total, failed, changed, hasStaleInWidget, hardClear: needsHardClear }); } catch (_) {}
            return Boolean(rendered);
          }
          const fileInputs = Array.from(targetForm.querySelectorAll('.t-input-group_uw input[type="file"], input[type="file"]'));
          if (!fileInputs.length) return false;
          return Boolean(syncFilesToAvailableInputs(fileInputs, trackedFiles));
        }, { form: targetForm, values: preservedFormValues });
      } finally {
    ...
    function scheduleCartUploadCleanup(delayMs = 260) {
      ...
      cartUploadCleanupTimer = window.setTimeout(() => {
        if (orderSubmitInFlight) {
          console.warn('[CCK-DEL] проверку отложил — идёт оформление (orderSubmitInFlight).');
    ...
    let cartCleanupClickBound = false;
    function bindCartCleanupClickTrigger() {
      if (cartCleanupClickBound) return;
      cartCleanupClickBound = true;
      // Любой клик внутри корзины Tilda (в т.ч. кнопка удаления товара) — надёжнее
      // MutationObserver: перепроверяем состав корзины несколько раз, пока Tilda
      // не «устаканит» удаление (обновит tcart.products и DOM).
      document.addEventListener('click', (event) => {
        const target = event.target;
        if (!(target instanceof Element)) return;
        if (!target.closest('.t706__cartwin, .t706__cartpage, .t706')) return;
        console.warn('[CCK-DEL] Клик в корзине Tilda — планирую перепроверку состава (300/1000/2500 мс).');
        [300, 1000, 2500].forEach((delayMs) => window.setTimeout(() => scheduleCartUploadCleanup(0), delayMs));
      }, true);
    }
    function ensureCartUploadCleanupObserver(cartWindow = getTildaCartWindow()) {
      bindCartCleanupClickTrigger();
      if (typeof MutationObserver === 'undefined') return;
      // Наблюдаем за СТАБИЛЬНЫМ контейнером корзины (.t706), а не за самим окном —
      // Tilda при удалении товара может пересоздать узел окна, и наблюдатель,
      // привязанный к старому узлу, перестаёт получать события (удаление не
      // отлавливается → файлы/описание удалённого товара остаются в заказе).
      const observeTarget = (cartWindow && cartWindow.closest && cartWindow.closest('.t706'))
        || document.querySelector('.t706')
        || cartWindow
        || document.body;
      if (!observeTarget) return;
      if (cartUploadMutationObserverTarget === observeTarget && cartUploadMutationObserver) return;
      if (cartUploadMutationObserver) {
        cartUploadMutationObserver.disconnect();
      }
      cartUploadMutationObserverTarget = observeTarget;
      console.warn('[CCK-DEL] Наблюдатель за корзиной подключён к', observeTarget?.className || observeTarget?.nodeName);
      cartUploadMutationObserver = new MutationObserver(() => {
        scheduleCartUploadCleanup();
      });
      cartUploadMutationObserver.observe(observeTarget, {
        childList: true,
        subtree: true,
        attributes: true,
...
function renderLeftSummary() {
    ...
    function hydrateTemplatePreviewIcons(root = document.getElementById('stepContent')) {
      ...
      slots.forEach((slot) => {
        ...
        // Для иконки на КАРТОЧКЕ предпочитаем cardImage (если задан),
        // а previewImage используется в композите рендера на изделии. Так можно
        // показать в списке шаблонов одну картинку (illustrative), а на
        // изделии впечатать другой PNG-исходник.
        const iconSrcRaw = (template && (template.cardImage || template.previewImage)) || '';
        if (template && iconSrcRaw) {
          // Не встраиваем data URL — это раздувает innerHTML (data URL могут
          // быть многомегабайтными, а на шаге 2 десятки шаблонов → итоговый
          // innerHTML на десятки мегабайт = синхронный парсинг ~300ms+).
          // Используем нормализованный URL (тот же, по которому отработал
          // preloadImageUrl) — иначе браузер делает повторный fetch с
          // редиректом www.dropbox.com → dl.dropboxusercontent.com.
          // referrerpolicy=no-referrer — Dropbox блокирует in-app браузеры
          // (Telegram/Instagram WebView) по referrer; без этого превью не
          // загружаются и видны битые иконки.
          // onerror — если всё же не загрузилось, прячем элемент, чтобы не
          // показывать вопросительный знак iOS placeholder.
          const displayUrl = normalizeProductImageUrl(iconSrcRaw);
          art.innerHTML = `<img src="${escapeAttr(displayUrl)}" alt="${escapeAttr(template.name)}" class="template-choice-icon-preview-img" loading="eager" decoding="async" referrerpolicy="no-referrer" onerror="this.style.visibility='hidden'" />`;
      ...
      pendingSlots.forEach(({ slot, art, template, promise }) => {
        promise.then((dataUrl) => {
          if (hydrationToken !== templatePreviewHydrationToken) return;
          const fallbackUrl = normalizeProductImageUrl(template.previewImage);
          art.innerHTML = `<img src="${escapeAttr(dataUrl || fallbackUrl)}" alt="${escapeAttr(template.name)}" class="template-choice-icon-preview-img" referrerpolicy="no-referrer" onerror="this.style.visibility='hidden'" />`;
          slot.classList.remove('is-loading');
          slot.classList.add('is-ready');
        });
      });
    ...
    function getProductCardPreviewImage(productId) {
      // Возвращаем нормализованный URL — тот же, по которому отработал
      // preloadImageUrl. Иначе при подстановке в <img src> браузер делает
      // повторный fetch с редиректом www.dropbox.com → dl.dropboxusercontent.com.
      const rawUrl = getProductPreviewImage(
        productId,
      ...
      return rawUrl ? normalizeProductImageUrl(rawUrl) : '';
    }
    ...
    function syncGarmentLoadingOverlay() {
      ...
      // The overlay lives inside #garment in HTML, but #garment is hidden
      // (display:none) when the Konva flat preview is rendered. Move the
      // overlay up to #garmentWrap once so it stays visible during product /
      // template / color switches and bump z-index above flatPreview (z:2)
      // and konvaPreviewContainer (z:15).
      if (overlay && garmentWrap && overlay.parentElement !== garmentWrap) {
        garmentWrap.appendChild(overlay);
        overlay.style.zIndex = '30';
      }
      const overlayText = overlay?.querySelector('.garment-loading-text');
      const isLoading = garmentLoadingState.base || garmentLoadingState.preview || garmentLoadingState.flatPreview;
      const isPreviewLoading = garmentLoadingState.preview || garmentLoadingState.flatPreview;
      if (garment) {
        garment.classList.toggle('is-base-loading', isLoading);
      }
      if (garmentWrap) {
        garmentWrap.classList.toggle('is-base-loading', isLoading);
      }
      if (overlay) {
        overlay.setAttribute('aria-hidden', isLoading ? 'false' : 'true');
        // Inline opacity overrides the CSS rule that keys off `.garment.is-base-loading`,
        // which no longer reaches the overlay after we relocated it.
        overlay.style.opacity = isLoading ? '1' : '0';
      }
      if (overlayText) {
        overlayText.textContent = isPreviewLoading
          ? garmentLoadingState.previewLabel
          : GARMENT_LOADING_LABELS.base;
      }
    }

    function setGarmentFlatPreviewLoading(isLoading, label = GARMENT_LOADING_LABELS.template) {
      garmentLoadingState.flatPreview = Boolean(isLoading);
      if (label) {
        ...
        return `
          <button class="option-btn product-choice-card ${state.product === item.id ? 'active' : ''}" data-product="${item.id}" type="button">
            <div class="product-choice-visual">
              ${previewUrl ? `<img data-product-preview-img="${item.id}" data-current-src="${previewUrl}" src="${previewUrl}" alt="${escapeAttr(item.name)}" referrerpolicy="no-referrer" onerror="this.style.visibility='hidden'">` : ''}
            </div>
    ...
    function updateStepOneActiveSelectionOnly() {
      // Лёгкий путь для смены ЦВЕТА / ШВА / РАЗМЕРА: набор swatch'ей не
      // меняется (изделие то же), поэтому пересборка variantsBlock outerHTML
      // и повторный bindStepEvents не нужны — это давало 60-100мс long-task
      // при каждом клике по цвету. Здесь только: переключаем active-класс,
      // ре-рендерим Konva flatPreview (cache hit ⇒ синхронно из памяти),
      // обновляем цену.
      const container = document.getElementById('stepContent');
      if (!container || state.step !== 1) {
        render();
      ...
      container.querySelectorAll('[data-seam]').forEach((btn) => {
        btn.classList.toggle('active', btn.dataset.seam === state.seam);
      });
      container.querySelectorAll('[data-size]').forEach((btn) => {
    ...
    function updateStepTwoActiveSelectionOnly() {
      // Лёгкий путь для смены ШАБЛОНА на step 2 (в пределах того же таба).
      // Без render() / renderPreservingStepContentScroll — иначе stepPanel
      // .innerHTML пересоздаётся, и <img> карточек шаблонов проходят
      // повторный decode (визуально «рендерятся по новой»).
      // Здесь только: переключаем active-класс, ре-рендерим превью изделия,
      // обновляем цену.
      const container = document.getElementById('stepContent');
      if (!container || state.step !== 2) {
        render();
        return;
      }
      container.querySelectorAll('[data-template]').forEach((btn) => {
        btn.classList.toggle('active', btn.dataset.template === state.template && !state.selectedCustom);
      });
      container.querySelectorAll('[data-custom]').forEach((btn) => {
        btn.classList.toggle('active', btn.dataset.custom === state.selectedCustom);
      });
      renderVisualizer();
      renderLivePrice();
      renderButtons();
    ...
    function stepOneVariantSetSignature(productId = state.product, colorName = state.color) {
      // Сигнатура зависит только от того, какой набор switch'ей будет в DOM.
      // Если до и после клика сигнатуры совпадают — full rebuild не нужен.
      const seams = (typeof getAvailableSeams === 'function') ? getAvailableSeams() : [];
      const sizes = (typeof getSizesByProduct === 'function') ? getSizesByProduct(productId) : [];
      return JSON.stringify({
        product: productId,
    ...
    function updateStepFourControls(options = {}) {
      ...
      if (textInput && textInput.value !== state.textValue && (options.syncTextInput || !isTextInputFocused)) {
    ...
    function createStepButton(stepDef) {
      ...
      const isUploadSkipped = shouldSkipUploadStep() && stepDef.id === 3;
      const disabled = !unlocked || isCustomBlocked || isUploadSkipped;
      ...
      btn.addEventListener('click', () => {
        ...
        if (shouldSkipUploadStep() && stepDef.id === 3) {
          openModal('uploadBlockedModal');
          return;
        }
        mobileStepNavScrollPending = true;
    ...
    function refreshStepNavState() {
      document.querySelectorAll('#stepList .step-btn, #stepListMobile .step-btn').forEach((btn) => {
        ...
        const isUploadSkipped = shouldSkipUploadStep() && stepId === 3;
        const disabled = !unlocked || isCustomBlocked || isUploadSkipped;
    ...
    function getDesignLayerRenderState() {
      ...
      if (!hideDesign && state.selectedCustom) {
        ...
        if (customTpl && customTpl.previewImage) {
          const tplDefaults = customTpl.photoDefaults || null;
          let boxStyle = 'left:50%; top:38.64%; width:8.8%; height:8.83%;';
          if (tplDefaults) {
            const centerLeftPct = ((tplDefaults.exportX + tplDefaults.width / 2) / 1000) * 100;
            const topPct = (tplDefaults.exportY / 1250) * 100;
            const widthPct = (tplDefaults.width / 1000) * 100;
            const heightPct = (tplDefaults.height / 1250) * 100;
            boxStyle = `left:${centerLeftPct}%; top:${topPct}%; width:${widthPct}%; height:${heightPct}%;`;
          }
          html = `
            <div
              class="image-box"
              style="${boxStyle} border:none; border-radius:0; background:transparent;"
            >
              <img src="${escapeAttr(normalizeProductImageUrl(customTpl.previewImage))}" alt="preview" referrerpolicy="no-referrer" onerror="this.style.visibility='hidden'" style="object-fit:contain; object-position:center center; width:100%; height:100%; pointer-events:none; user-select:none;">
            </div>
          `;
        }
      } else if (!hideDesign && isPhotoTextTemplate(selectedTemplate) && templatePhotoLayout) {
        const textBlockHtml = `
          <div class="emb-block" style="--print-scale:${currentScale}; --text-color:${state.textColor}; --preview-text-max-width:${previewTextStyle.maxWidth}; --preview-text-line-height:${previewTextStyle.lineHeight}; --preview-text-font-weight:${previewTextStyle.fontWeight}; --preview-text-letter-spacing:${previewTextStyle.letterSpacing}; --preview-text-top:${previewTextStyle.previewTop}; --preview-text-transform:${previewTextStyle.textTransform}; left:${previewTextStyle.left}; right:${previewTextStyle.right}; width:${previewTextStyle.width}; max-width:${previewTextStyle.maxWidth}; transform:${previewTextStyle.transform}; transform-origin:${previewTextStyle.transformOrigin}; text-align:${previewTextStyle.textAlign}; top:${previewTextStyle.previewTop}; line-height:${previewTextStyle.lineHeight}; font-weight:${previewTextStyle.fontWeight}; letter-spacing:${previewTextStyle.letterSpacing}; text-transform:${previewTextStyle.textTransform}; font-size:${previewTextStyle.fontSize}; font-family:${previewFontFamily}; -webkit-line-clamp:${previewTextRules.maxLines};${previewTextStyle.fontSizeAdjust != null ? ` font-size-adjust:${previewTextStyle.fontSizeAdjust};` : ''}">
            ${previewTextHtml}
          </div>
        `;
      ...
      const key = JSON.stringify({
        step: state.step,
        hideDesign,
        selectedCustom: state.selectedCustom || false,
    ...
    function renderVisualizer() {
      ...
      const baseImageUrl = getCurrentProductImage();
      if (baseImage) {
        // Debounce 150мс на показе спинера: при прогретом кеше (а с
        // preloadInitialRenderImages он почти всегда прогрет) decode() резолвится
        // за пару миллисекунд, спинер просто не успевает появиться. Если же
        // картинка реально холодная — спинер появится через 150мс и будет
        // крутиться корректно.
        syncDeferredImageSource(baseImage, baseImageUrl, {
          setLoading: (isLoading) => {
            if (isLoading) {
              if (baseImageSpinnerTimer) clearTimeout(baseImageSpinnerTimer);
              baseImageSpinnerTimer = setTimeout(() => {
                baseImageSpinnerTimer = null;
                setGarmentBaseImageLoading(true);
              }, 150);
            } else {
              if (baseImageSpinnerTimer) {
                clearTimeout(baseImageSpinnerTimer);
                baseImageSpinnerTimer = null;
              }
              setGarmentBaseImageLoading(false);
            }
          },
        });
      }
      ...
      if (isKonvaAvailable()) {
        ...
        const sceneKey = JSON.stringify({
          step: state.step,
          kind: selectedTemplate?.kind || '',
          custom: state.selectedCustom || false,
          ...
          cropZoom: state.cropZoom,
          photoGrayscale: state.photoGrayscale,
          // КРИТИЧНО: без baseImageSrc/scale смена цвета или изделия не
          // меняет ключ → flatPreview не перерисовывается, но #garment всё
          // равно скрыт → пользователь видит «зависание».
          baseImageSrc: sceneParams.baseImageSrc,
          baseImageScale: sceneParams.baseImageScale,
        });

        // На шаге 4 используем live Konva canvas вместо flat PNG. Когда юзер
        // вводит текст, в обычном режиме flatPreview.src менялся на новый
        // PNG → браузер сбрасывал свой visual viewport zoom (видимый pinch).
        // В live-canvas режиме <canvas> DOM-элемент тот же, текст
        // обновляется через Konva API → DOM identity сохраняется → zoom не
        // сбрасывается. Также текст рендерится мгновенно без debounce.
        const useLiveCanvas = (state.step === 4);
        if (useLiveCanvas) {
          // Live Konva canvas — рендерит ПОЛНУЮ сцену (base image + text +
          // photo + frame). DOM base-img скрываем, canvas покрывает всё.
          // Так step 4 визуально идентичен step 5 (тот же масштаб t-shirt),
          // но через canvas вместо PNG → text update без img.src замены →
          // browser pinch-zoom не сбрасывается.
          const devPromises = Promise.all([
            sceneParams.customPreviewSrc ? fetchImageAsDataUrl(sceneParams.customPreviewSrc) : Promise.resolve(null),
            sceneParams.baseImageSrc ? fetchImageAsDataUrl(sceneParams.baseImageSrc) : Promise.resolve(null),
            sceneParams.frameSrc ? fetchImageAsDataUrl(sceneParams.frameSrc) : Promise.resolve(null),
          ]);
          devPromises.then(([devCustomPreviewDataUrl, baseImageDataUrl, frameDataUrl]) => {
            const overlayParams = {
              ...sceneParams,
              baseImageSrc: baseImageDataUrl || sceneParams.baseImageSrc,
              frameSrc: frameDataUrl || sceneParams.frameSrc,
              customPreviewSrc: devCustomPreviewDataUrl || sceneParams.customPreviewSrc,
            };
          ...
          // Прячем garment (base-img в DOM не нужен — base рисуется в canvas).
          // Используем класс, потому что inline style display:none затирается
          // syncGarmentScale, который вызывается ниже.
          const garmentWrapEl = document.getElementById('garmentWrap');
          if (garmentWrapEl) garmentWrapEl.classList.add('canvas-mode');
          const flatPreview = document.getElementById('konvaFlatPreview');
          if (flatPreview) flatPreview.style.display = 'none';
        } else if (KONVA_DEV_MODE) {
          // Legacy dev mode (для отладки) — оставляем как было (overlay only).
          const devCustomPreviewPromise = sceneParams.customPreviewSrc
            ? fetchImageAsDataUrl(sceneParams.customPreviewSrc)
            : Promise.resolve(null);
          devCustomPreviewPromise.then((devCustomPreviewDataUrl) => {
            const overlayParams = { ...sceneParams, baseImageSrc: null, customPreviewSrc: devCustomPreviewDataUrl || sceneParams.customPreviewSrc };
            const scene = konvaResolveScene(overlayParams);
            konvaRenderPreview(scene, sceneKey).catch((err) => {
              console.warn('Konva preview render failed, falling back to legacy.', err);
              konvaDestroyPreview();
          ...
          const flatPreviewDev = document.getElementById('konvaFlatPreview');
          if (flatPreviewDev) flatPreviewDev.style.display = 'none';
          const garmentWrapElDev = document.getElementById('garmentWrap');
          if (garmentWrapElDev) garmentWrapElDev.classList.remove('canvas-mode');
        } else {
          // Production mode: render full scene to static image (base + elements)
          konvaDestroyPreview();
          garment.style.display = 'none';
          const garmentWrapElProd = document.getElementById('garmentWrap');
          if (garmentWrapElProd) garmentWrapElProd.classList.remove('canvas-mode');
          // Ensure flat preview <img> exists
          let flatPreview = document.getElementById('konvaFlatPreview');
          if (!flatPreview) {
            flatPreview = document.createElement('img');
            flatPreview.id = 'konvaFlatPreview';
            flatPreview.style.cssText = 'position:absolute; inset:0; width:100%; height:100%; object-fit:contain; object-position:center; z-index:2; pointer-events:none; user-select:none;';
            flatPreview.alt = '';
            // referrerpolicy + onerror — для in-app браузеров (Telegram/Instagram WebView),
            // которые блокируют Dropbox по referrer и иначе показывают «битую картинку».
            flatPreview.referrerPolicy = 'no-referrer';
            flatPreview.onerror = () => { flatPreview.style.visibility = 'hidden'; };
            const garmentWrap = document.getElementById('garmentWrap');
            if (garmentWrap) garmentWrap.appendChild(flatPreview);
          }
          flatPreview.style.display = '';
          // Only re-render if scene changed
          if (sceneKey !== konvaPreviewLastKey) {
            konvaPreviewLastKey = sceneKey;

            // Fast path: эту сцену уже рендерили — отдаём из кеша синхронно,
            // без сетевых запросов и без спинера.
            const cachedDataUrl = konvaFlatPreviewCache.get(sceneKey);
            if (cachedDataUrl) {
              flatPreview.src = cachedDataUrl;
              // Аннулируем любой висящий рендер от предыдущего sceneKey,
              // чтобы он не перезаписал свежий src.
              konvaFlatPreviewRenderToken += 1;
            } else {
            const renderToken = ++konvaFlatPreviewRenderToken;
            // С прогретым кешем (preloadInitialRenderImages) переключение чаще
            // всего длится десятки миллисекунд — спинер не должен мигать.
            // Поэтому показываем его только если рендер длится дольше 150 мс.
            const spinnerTimer = setTimeout(() => {
              if (renderToken === konvaFlatPreviewRenderToken) {
                setGarmentFlatPreviewLoading(true, GARMENT_LOADING_LABELS.template);
              }
            }, 150);
            (async () => {
              try {
                await waitForCurrentBaseImageReady().catch(() => {});
                const [baseImageDataUrl, frameDataUrl, customPreviewDataUrl] = await Promise.all([
                  sceneParams.baseImageSrc ? fetchImageAsDataUrl(sceneParams.baseImageSrc) : Promise.resolve(null),
                  sceneParams.frameSrc ? fetchImageAsDataUrl(sceneParams.frameSrc) : Promise.resolve(null),
                  sceneParams.customPreviewSrc ? fetchImageAsDataUrl(sceneParams.customPreviewSrc) : Promise.resolve(null),
                ]);
                if (renderToken !== konvaFlatPreviewRenderToken) return;
                const exportParams = {
                  ...sceneParams,
                  baseImageSrc: baseImageDataUrl || sceneParams.baseImageSrc,
                  frameSrc: frameDataUrl || sceneParams.frameSrc,
                ...
                if (renderToken !== konvaFlatPreviewRenderToken) return;
                if (dataUrl) {
                  konvaFlatPreviewCache.set(sceneKey, dataUrl);
                }
                if (dataUrl && flatPreview) {
                  flatPreview.src = dataUrl;
                  if (typeof flatPreview.decode === 'function') {
                    try { await flatPreview.decode(); } catch (_) {}
                  }
                }
              } catch (err) {
                console.warn('Flat preview render failed:', err);
                if (renderToken !== konvaFlatPreviewRenderToken) return;
                // Fallback: show garment + legacy
                garment.style.display = '';
                if (flatPreview) flatPreview.style.display = 'none';
                if (designLayer) designLayer.style.display = '';
                renderLegacyDesignLayer();
                ...
                if (renderToken === konvaFlatPreviewRenderToken) {
                  setGarmentFlatPreviewLoading(false);
                }
              }
            })();
            } // end else (cache miss)
      ...
      // editable-класс ставит touch-action: none + user-select: none на garment-wrap.
      // Без него браузерный native pinch-zoom перехватывает жесты на мобиле,
      // и наш pointer-handler (который сохраняет state.cropZoom) не получает
      // multi-touch события. После изменения текста / цвета текста layout
      // пересчитывается → браузер сбрасывает свой visual pinch-zoom →
      // пользователь видит «zoom reset». Поэтому editable также включён
      // на step 4, когда есть фото для редактирования.
      const allowPreviewEditing = (state.step === 3 || state.step === 4) && hasActiveUpload && !isCustomSelected();
      ...
      garmentWrap.classList.toggle('preview-touch-zoom', false);
      garmentWrap.classList.toggle('show-gesture-hint', false);
    ...
    function render() {
      ...
      if (shell) shell.classList.toggle('step-4-mode', state.step === 4);
    ...
    function scheduleFullCatalogImagePreload() {
      ...
      preloadCustomTemplatePreviewImages();
    }

    // Прогрев flat-preview кеша для всех (изделие × цвет × шов) комбинаций.
    // После прогрева переключения в шаге 1 (и любые повторные комбинации)
    // мгновенные — данные тянутся из konvaFlatPreviewCache синхронно.
    async function pregenerateOneFlatPreview(productId, colorName, seamName) {
      // 1) Синхронно подменяем state, строим sceneParams + sceneKey, возвращаем
      //    state — асинхронная часть работает с локальными копиями и больше
      //    глобальный state не читает.
      const saved = { product: state.product, color: state.color, seam: state.seam };
      let sceneParams;
      let sceneKey;
      try {
        ...
        state.seam = seamName;
        sceneParams = buildKonvaSceneParams();
        const selectedTemplate = getSelectedTemplate();
        ...
        state.seam = saved.seam;
      }
      if (konvaFlatPreviewCache.has(sceneKey)) return;
      ...
      try {
        ...
        console.warn('Pregenerate flat preview failed:', err);
      }
    }

    function pregenerateAllFlatPreviews(options = {}) {
      // Возвращает Promise, который резолвится, когда прогрет flat-preview кеш для
      // всех (изделие × цвет × шов) комбинаций. Используется и в инициализации
      // (await — чтобы после первого спиннера всё уже было в кеше и не было
      // зависаний при кликах), и для повторного догрева в фоне.
      if (typeof isKonvaAvailable === 'function' && !isKonvaAvailable()) return Promise.resolve();
      ...
      productIds.forEach((productId) => {
        ...
        variants.forEach((variant) => {
          ...
          const seams = Array.isArray(variant?.seams) ? variant.seams : [];
          if (!seams.length) {
            tasks.push({ productId, colorName, seamName: '' });
          } else {
            seams.forEach((seam) => {
              tasks.push({ productId, colorName, seamName: seam?.name || '' });
            });
          }
        });
      });

      const concurrency = Math.max(1, Math.min(options.concurrency || 2, tasks.length));
      ...
      const runNext = async () => {
        while (index < tasks.length) {
          ...
          await pregenerateOneFlatPreview(task.productId, task.colorName, task.seamName);
          completed++;
          if (onProgress) {
    ...
    function pregenerateAllFlatPreviewsInBackground(options = {}) {
      // Не блокирующий вариант — fire-and-forget, для повторного догрева кеша
      // (например, после загрузки пользовательских шаблонов).
      pregenerateAllFlatPreviews(options).catch(() => {});
    }

    async function pregenerateCurrentVariantForAllSteps(options = {}) {
      // Cache-key flat-preview включает state.step, поэтому переход между
      // шагами требует отдельного прогрева. Здесь прогреваем текущее изделие/
      // цвет/шов для всех шагов 1..6, чтобы переход «Изделие → Шаблон дизайна
      // → ...» был мгновенным без CPU-зависания на canvas-экспорте.
      if (typeof isKonvaAvailable === 'function' && !isKonvaAvailable()) return;
      ...
      try {
        for (let step = 1; step <= total; step++) {
          state.step = step;
          await pregenerateOneFlatPreview(state.product, state.color, state.seam);
          if (onProgress) {
            try { onProgress(step, total); } catch (e) {}
          }
    ...
    async function initializeConfigurator() {
      ...
      try { window.__cckLog?.('configurator_init_start', { orderId: state.orderId, step: state.step }); } catch (_) {}
      try {
        ...
        await preloadInitialRenderImages();
        render();
        // Подключаем слежение за корзиной СРАЗУ, а не только при оформлении: клиент
        // может открыть корзину сам (через иконку Tilda), удалить товар — и без
        // глобального слежения это изменение не отлавливалось.
        bindCartCleanupClickTrigger();
        ensureCartUploadCleanupObserver();
        // Полный прогрев каталога ДО скрытия первого спиннера — чтобы после
        // его исчезновения не было сетевых дозагрузок и фонового CPU-экспорта
        // канвасов, которые вызывают ощущение «постоянной подгрузки» при
        // переключении изделия/цвета/шва.
        scheduleFullCatalogImagePreload();
        const loadingTextEl = document.querySelector('#shellLoadingOverlay .shell-loading-text');
        const baseLoadingText = loadingTextEl ? loadingTextEl.textContent : '';
        // 1) Прогрев всех (изделие × цвет × шов) на текущем шаге — мгновенные
        //    переключения цвета/изделия без сетевых запросов.
        await pregenerateAllFlatPreviews({
          concurrency: 2,
          onProgress: (done, total) => {
            if (!loadingTextEl || !total) return;
            loadingTextEl.textContent = `Готовим превью ${done}/${total}`;
          },
        });
        // 2) Прогрев текущего варианта для всех 6 шагов — мгновенный переход
        //    «Шаг 1 → Шаг 2 → ...» без зависания на canvas-экспорте.
        await pregenerateCurrentVariantForAllSteps({
          onProgress: (done, total) => {
            if (!loadingTextEl) return;
            loadingTextEl.textContent = `Готовим шаги ${done}/${total}`;
          },
        });
        if (loadingTextEl && baseLoadingText) loadingTextEl.textContent = baseLoadingText;
      } finally {
        requestAnimationFrame(() => {
          setShellLoading(false);
          // Auto-restore without modal — no "unsaved progress" popup
          const restored = Boolean(pendingRestoreSnapshot);
          ...
          try {
            window.__cckLog?.('configurator_init_complete', {
              orderId: state.orderId,
              step: state.step,
